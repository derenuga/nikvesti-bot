...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            # Чекаємо не на сам блок, а на ВІДСОТОК у ньому. Блок #me-ring
            # малюється одразу, а підпис під ним заповнюється лише після
            # відповіді /api/kpi (він іде в MySQL сайту і думає секунди). На
            # швидкій машині різниці не видно, а на раннері GitHub перевірка
            # встигала прочитати порожній підпис — і падала «на рівному місці»
            # (прогін #19, 16.08). Гонку сюди приніс я цим самим поверненням
            # на Головну; чекати треба на дані, а не на елемент.
            await page.wait_for_selector("#me-ring .me-cap b", timeout=5000)
