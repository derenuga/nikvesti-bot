...
ORG_STOP = {
    ...
    "від", "про", "питань", "справах", "український", "українська",
    "області", "область", "обл", "м",
}
# «України» — НЕ службове слово, і це рішення Олега 16.08 («навіщо ти знову
# зібрався зʼєднувати МВС?»). «Міністерство внутрішніх справ» без країни може
# бути польським, російським, чиїм завгодно, і тихо чіпляти таку згадку до
# українського міністерства — та сама монетка за людину, якої ми уникали в
# ручному розборі (11451, 8122, 26684 не пішли в пакети саме тому).
#
# Викидаємо його рівно там, де орган УЖЕ прив'язаний топонімом: «ГУ ДСНС
# України у Миколаївській області» = «ГУ ДСНС у Миколаївській області», бо
# область названо, і слово «України» нічого не додає. Немає топоніма — слово
# лишається значущим.
# Форма після відсікання закінчення — саме вона потрапляє в набір слів.
ORG_COUNTRY = {"україни", "україна", "україн"}
...
def org_key(name):
    ...
    if not words:
        return None
    # «України» лишається в ключі, поки орган не прив'язаний топонімом
    if any(w in ORG_TOPONYMS.values() for w in words):
        words = [w for w in words if w not in ORG_COUNTRY]
        if not words:
            return None
