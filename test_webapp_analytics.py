...
PROJECTS = [{
    ...
    "deadlines": [{"id": 5, "kind": "narrative", "stage": "interim", "title": "",
                   "due": _in(9), "assignee": "Катерина Середа",
...
DASH = {
    ...
    "newsroom": {
        ...
        # сервер уже впорядкував за вагою (власне × own_weight) і звів автора до
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            check("вага підписана числом із сервера, а не зашитим у фронт",
                  "3,5" in await page.inner_text("#an-data"))
            ...
            check("картка з підписниками НЕ каже «зрізу немає» — це була брехня "
                  "поверх видимих чисел",
                  "зрізу за цей період ще немає" not in soc
                  and "лише підписники" in soc)
