...
function impactTimelineItem(a, ro) {
  ...
  const badges = [
    ...
    // Олег, 14.08: «можна це „зовнішня публікація“ замінити просто іконкою?»
    // Так: значок сайту вже каже, що матеріал чужий, а домен каже — чий.
    // Глобус лежить ПІСЛЯ favicon і ховається, поки той живий: коли значок
    // не завантажився, onerror прибирає img — і глобус проступає сам
    a.external ? `<span class="itl-badge">${fav}${icon("globe", "ic itl-globe")}${
      esc(a.source || "зовнішнє джерело")}</span>`
...
/* fixedColor — коли смуга має бути одного кольору незалежно від результату.
   Так зроблено на екрані журналістки (див. PACE_COLOR): своє фото в кільці
   кольору «ти відстаєш» демотивує, а не інформує — вона й так бачить цифри.
   Шкала червоний→жовтий→зелений лишається менеджерському «Звіту». */
...
/* Підекрани журналістки: сюди менеджерський перегляд ХОДИТЬ, не втрачаючи,
   на кого дивиться. Банку тем тут бракувало — і саме тому «З моїх новин»
   показував нуль: щойно з перегляду відкривали банк, previewPerson
   обнулявся (див. nav), і сервер рахував того, ХТО дивиться. */
const J_SUBVIEWS = new Set(["preview", "mypubs", "myhist", "mydone", "myfeed",
                            "myimpacts", "impact",
                            "promises", "promise", "dupes"]);
...
/* Колір кільця на ВЛАСНОМУ екрані. Тут він НЕ оцінює людину. Червоного не було
   ніколи, а бурштинове відставання (28.07 — «підтягнись, а не провалила»)
   Олег 11.08 скасував: своє фото в жовтому кільці читається як вирок, хоч би
   що означав колір у коді. Темп і без кольору сказаний двічі — засічкою «де ти
   маєш бути сьогодні» і фразою («Час додати темпу 💪»).

   Лишилось два значення, і кожне однозначне: синє — ти в роботі, зелене —
   норму ЗАКРИТО. Тому `ahead` теж синій: іти із запасом — ще не закрити, а
   зелений мусить означати рівно одне (те саме правило, що у смузі місяців
   картки перегляду). Оцінка кольором лишається там, де вона про роботу
   відділу, а не про людину, — на менеджерському «Звіті». */
const PACE_COLOR = {
  done: "var(--good)",
  ahead: "var(--blue)", on: "var(--blue)", start: "var(--blue)",
  behind: "var(--blue)",
};
