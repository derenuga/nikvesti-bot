...
handlers/
  ...
  fb_missing.py           — монітор власних новин без Facebook: щогодини (9–21) дивиться свіжі own-новини (nodes, вікно [now−24год, now−3год]) і тими ж get_fb_stats, що /stat, перевіряє, чи є пост у ФБ; якщо ні — РАЗ підказує редакції (лінк + автор із TG-хендлом за TEAM) і дає готову чернетку поста окремим блоком коду (json-ld title + description + лінк). Перший запуск — тихий baseline. /fbmissing (прогнати зараз), /fbmissing_test (прев'ю формату)
  ...
  stat.py                 — /stat <url>: статистика матеріалу (Facebook + Instagram + TikTok + Telegram + GA4)
  ...
  viber_mirror.py         — дзеркало Telegram→Viber: кожен пост каналу @nikvesti (крім репостів/службових) автопоститься у Viber-канал через Channels Post API (звільнити журналістів від крос-постингу). Текст+лінк, фото (picture, найбільший розмір ≤1 МБ), відео (video ≤20 МБ через getFile Telegram, ≤180 с; крупніше — текстом); підпис до відео окремим повідомленням (Viber video підпису не має); будь-який збій медіа → фолбек на текст. /viber_setup, /viber_test; env VIBER_AUTH_TOKEN (+ VIBER_WEBHOOK_URL)
...
| /stat \<url\> | Статистика матеріалу (Facebook + Instagram + TikTok + Telegram + GA4) за URL nikvesti.com. Instagram і TikTok шукаються семантично (по підпису, бо URL у стрічці немає); блок TikTok — лише коли налаштовано OAuth |
