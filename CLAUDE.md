...
handlers/
  ...
  team_roster.py          — ростер команди для Mini App «Команда»: відділи Newsroom/Creative (KPI-групи; «керівництво» — службова група менеджерів), прапорець менеджера, резолв initData → людина (кеш team_users → відомі id → username), кеш tg_id для пінгів
  team_tasks.py           — creative tasks і тематики проєктів у Норі (team_creative_tasks/team_project_themes/team_task_events): завдання від редактора (людина × тип news/article × проєкт CMS або позапроєктне × тематика × к-сть × нотатка), статуси open→done/dropped, пінг виконавиці best-effort
  team_kpi.py             — рекурентні KPI команди (team_kpi_norms/team_kpi_overrides у Норі): норма на відділ (метрика news/article × період тиждень/місяць × ціль на людину), правки по людині на поточний період (інша ціль / 0 = звільнена з нотаткою), факт сам із nodes по owner_id (матч ПІБ до users, кеш 5 хв), періоди за Києвом (тиждень з пн)
