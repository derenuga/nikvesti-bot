...
handlers/
  ...
  fb_missing.py           — монітор власних новин без Facebook: щогодини (9–21) дивиться свіжі own-новини (nodes, вікно [now−24год, now−3год]) і тими ж get_fb_stats, що /stat, перевіряє, чи є пост у ФБ; якщо ні — РАЗ підказує редакції (лінк + автор із TG-хендлом за TEAM) і дає готову чернетку поста окремим блоком коду (json-ld title + description + лінк). Гейт «час настав»: nodes.published бреше для запланованих в адмінці новин (буває минулим до фактичного виходу), тому перед алертом читає сторінку статті й звіряє JSON-LD datePublished — час не настав (+ грейс 3 год від реального виходу) або сторінка не читається → відкласти без марку до наступної години. Перший запуск — тихий baseline. /fbmissing (прогнати зараз), /fbmissing_test (прев'ю формату)
  ...
  social_sheet.py         — місячний знімок аналітики в Google-таблицю «Аналітика МикВісті» (SOCIAL_SPREADSHEET_ID): лист на рік, місяці рядками, блоки Сайт (GA4 + SC Search/News/Discover) / Facebook / Instagram / Telegram (парсинг t.me, дзеркала TG_WEB_BASES з фолбеком telegram.me) / YouTube (Analytics API через OAuth — перегляди й години перегляду, /youtube_backfill за всю історію) / TikTok (Display API) / Viber (get_account_info — підписники, ЄДИНА метрика API; «Постів у канал» — власний лічильник дзеркала Telegram→Viber; «Активні користувачі» лишаються ручними, як «Охоплення» TikTok — API не віддає); старі листи добудовуються самі; дельти MoM/підсумки року (останнє відоме для підписників, LOOKUP(9^99))/YoY/спарклайни — живі формули, стрілки ▲▼ — custom number format, вбудовані графіки; авто 1-го числа 10:30, /sheet_snapshot, /sheet_backfill, /sheet_format, /sheet_migrate_legacy
  ...
  team_tasks.py           — creative tasks і тематики проєктів у Норі (team_creative_tasks/team_project_themes/team_task_events): завдання від редактора (людина × тип news/article/post(+платформа Telegram/Instagram)/будь-який × проєкт CMS або позапроєктне × тематика × к-сть × дедлайн × нотатка; снапшоти partner_name/project_name), статуси open→done/dropped, пінг виконавиці best-effort; тематика має формат (news/article/post/video/hybrid або без); до проєкту кріпиться папка Google Drive (team_project_drive)
  team_kpi.py             — рекурентні KPI команди (team_kpi_norms/team_kpi_overrides у Норі): норма на відділ (метрика news/article × усі/лише власні (own_material) × період тиждень/місяць × ціль на людину; тижнева й місячна — незалежні норми), правки по людині на поточний період (інша ціль / 0 = звільнена з нотаткою), факт сам із nodes по owner_id (матч ПІБ до users, кеш 5 хв), періоди за Києвом (тиждень з пн)
  ...
  webapp.py               — aiohttp-сервер Mini App поруч із polling (post_init, стартує лише з PORT): перевірка HMAC initData, API v2 (/api/bootstrap одним запитом: люди+фото, проєкти+тематики, таски; /api/kpi — зведення KPI за роллю; POST/PATCH tasks, themes, kpi/norms, kpi/override — менеджерські), обидві БД деградують окремо, статика webapp/, /health (придатний як VIBER_WEBHOOK_URL), команда /team
...
webapp/                   — SPA Mini App «Команда» без збірки (index.html + style.css + app.js), дизайн v2: біло-синьо-голуба гама, SVG-іконки Feather (без емодзі-кнопок), повітря. Редакторський флоу: головна — команда кружечками з розбивкою відкритих тасків по донорах, тап → персональний трекер людини (її KPI зі шторкою правки + таски по донорах); bottom-menu з «＋» → людина → форма creative task; таби Проєкти (список «донор великим + строки» з ручним порядком drag-n-drop (довгий натиск, team_project_order у Норі) / таймлайн-ґант зі смугами проєктів, липкими лейблами і точкою «сьогодні»; тематики з розкроєм квот; дедлайни звітності — наративні/фінансові проміжні/фінальні + майлстоуни (team_project_deadlines), підказка найближчого в списку, нагадування — беклог), KPI (норми по відділах → люди з прогрес-барами → шторка правки: інша ціль/звільнити/до норми), Команда. Картинки: ресайз .webp → фолбек оригінал → ініціали. Журналістка — «Мої KPI» (тиждень+місяць з фактом) і read-only список своїх тасків (її повний інтерфейс — наступний шар)
...
| /fbmissing | Прогнати монітор власних новин без ФБ зараз (ігнорує гейт годин, поважає «раз»); алерти — в чат, де викликано |
| /fbmissing_test | Прев'ю формату підказки+чернетки на найсвіжішій власній новині, БЕЗ зміни стану (можна ганяти скільки завгодно) |
...
| :20 (9–21) | Монітор власних новин без Facebook: свіжі own-новини (nodes) → перевірка ФБ (як /stat) → раз підказує редакції з чернеткою поста |
