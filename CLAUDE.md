...
handlers/
  ...
  viber_mirror.py         — дзеркало Telegram→Viber: кожен пост каналу @nikvesti (крім репостів/службових) автопоститься у Viber-канал через Channels Post API (звільнити журналістів від крос-постингу). Текст+лінк, фото (picture, найбільший розмір ≤1 МБ), відео (video ≤20 МБ через getFile Telegram, ≤180 с; крупніше — текстом); підпис до відео окремим повідомленням (Viber video підпису не має); будь-який збій медіа → фолбек на текст; альбом (media_group_id) склеюється в один пост (перше фото з підписом, решта пропускається — Viber альбомів не має). /viber_setup, /viber_test; env VIBER_AUTH_TOKEN (+ VIBER_WEBHOOK_URL)
