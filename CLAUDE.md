...
handlers/
  ...
  webapp.py               — aiohttp-сервер Mini App поруч із polling (post_init, стартує лише з PORT): перевірка HMAC initData, API v2 (/api/bootstrap одним запитом: люди+фото, проєкти+тематики, таски; POST/PATCH tasks і themes — менеджерські), обидві БД деградують окремо, статика webapp/, /health (придатний як VIBER_WEBHOOK_URL), команда /team
...
webapp/                   — SPA Mini App «Команда» без збірки (index.html + style.css + app.js), дизайн v2: біло-синьо-голуба гама, SVG-іконки Feather (без емодзі-кнопок), повітря. Редакторський флоу: bottom-menu з «＋» → людина (кружечки-фото з сайту) → форма creative task; таби Проєкти (список «донор великим + строки» / таймлайн-ґант зі смугами проєктів, липкими лейблами і точкою «сьогодні»; тематики з розкроєм квот), KPI (заглушка до рішення), Команда. Картинки: ресайз .webp → фолбек оригінал → ініціали. Журналістка — read-only список своїх тасків (її інтерфейс — наступний шар)
