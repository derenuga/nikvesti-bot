...
handlers/
  ...
  bot_db.py               — власна Postgres-БД бота (Railway): дзеркало архіву, tsvector FTS, sync_state, daily_stats (історія трафіку GA4), social_stats (тижневі зрізи соцмереж), article_stats (снімки /stat + object_id для швидкого шляху); таблиці team_* (Mini App «Команда») створюються ліниво у своїх модулях, не тут
