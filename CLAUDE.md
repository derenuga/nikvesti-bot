...
handlers/
  ...
  team_matching.py        — зарахування виконання creative tasks (концепт): БД сайту каже проєкт публікації, але не тематику → пре-фільтр без AI (відкриті таски автора в проєкті, сумісні за типом) + AI-суддя (Sonnet, tool use → matched_task_id/confidence/reasoning): high → авто, не-high → черга Каті. /match_test <url> — прогін на реальній статті, НІЧОГО не зараховує
