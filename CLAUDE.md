...
**Обмеження росту (storage.py):** seen-списки (`document_ids`, `competitor_ids`) капляться на 1000 ID на джерело — свіжі в хвості, обрізаються найстаріші (caller додає нові в кінець). Тендери старші 120 днів прюняться разом з `message_to_tender` (у `bulk_save`, щогодинний прогон Prozorro). `tg_posts` — кап 20000. `traffic_spikes` — 8 замірів на слот.
