...
живі сторінки, зняті 17.08.2026, стиснуті gzip'ом (по 130 КБ кожна):
  ...
  • 322739 — Арахамія про молоді роки, знято 22.08.2026. Тут хвіст цитати
    «— згадує Давид Арахамія»: дієслова «згадує» не було в переліку, і ім'я
    надрукувалось на слайді двічі — у цитаті й у підписі під нею. Через цю
    статтю розбір хвоста переписано з переліку слів на будову фрази.
...
def main():
    ...
    check("порожній записаний підпис нічого не додає й не віднімає",
          plain(named) == stat_instagram.make_scorer(dict(sig, caption=""))(named))
    # Сигнали лише ДОДАЮТЬСЯ максимумом, тож жоден допис не може від них
    # програти — гірше, ніж було, не стане нікому
    base_tokens = stat_instagram._norm_tokens(f"{sig['title']} {sig['lead']}")
    for cand in (named, blind, own_words, "Погода в Миколаєві: дощ"):
        check(f"оцінка не падає нижче базової: {cand[:28]}…",
              withhint(cand) >= stat_instagram._score(base_tokens, cand))

    # Підпис, що НЕСЕ заголовок, знаходиться без судді. До 22.08 не
    # знаходився: `_score` міряє заголовок і лід одним мішком, і найнадійніший
    # з можливих підписів давав 0.478 при порозі 0.5
    head_cap = f"{sig['title']} Деталі — у карусері."
    check("підпис із дослівним заголовком — впевнений збіг",
          plain(head_cap) >= stat_instagram.ACCEPT, f"{plain(head_cap):.3f}")
    check("а чужий заголовок таким збігом не стає",
          plain("Погода в Миколаєві на вихідні: синоптики обіцяють дощ")
          < stat_instagram.ACCEPT)
    check("короткий заголовок сигналом не працює (випадкове покриття)",
          stat_instagram._headline_hit(
              stat_instagram._norm_tokens("Обстріл Миколаєва"),
              "Обстріл Миколаєва: наслідки") == 0.0)
    ...
    # ---------- 12. Хвіст атрибуції: будова, а не список дієслів ----------
    #
    # Привід (Олег, 22.08, скріншот слайда): «— згадує Давид Арахамія» лишився
    # в тексті цитати, бо «згадує» не було в переліку дієслів, — і ім'я
    # надрукувалось двічі, у цитаті й у підписі під нею. Дієслова мовлення це
    # відкритий клас, тож перелік лікує один випадок і чекає наступного.
    print("\n— Хвіст «хто сказав» ріжеться за будовою фрази —")
    a739 = article_of("322739")
    for i, q in enumerate(a739["quotes"]):
        speech, _ = carousel.split_speech(q)
        check(f"322739 цитата {i}: хвіст атрибуції зрізано",
              not re.search(r"[—–]\s*\S+\s+(?:Давид|Арахамія|політик)", speech),
              speech[-70:])
    speech0, who0 = carousel.split_speech(a739["quotes"][0])
    check("«згадує Давид Арахамія» більше не в цитаті",
          "згадує" not in speech0, speech0[-60:])
    check("і став підписом", who0 == "Давид Арахамія", who0)
    check("цитата лишилась дослівною, лише коротшою",
          speech0.rstrip(".") in a739["quotes"][0], speech0[:60])

    # Те, що різати НЕ можна: без цих перевірок «структурне» правило
    # відгризало б кінець думки
    keep = [
        ("тире виділення без коми", "— Головне — люди, які лишились у місті."),
        ("продовження через кому+тире",
         "— Ми зробили все, що могли, — і навіть більше."),
        ("назва в лапках усередині цитати",
         "— Ми відкриємо «Зеленбуд», — і на цьому крапка."),
        ("питання в хвості", "— Скільки це триватиме, — а хто відповість?"),
        ("коротка цитата, тире тільки на початку", "— Так."),
    ]
    for name, raw in keep:
        speech, _ = carousel.split_speech(raw)
        check(f"не ріже: {name}",
              raw.lstrip("— ").rstrip(".") in speech, speech)

    # Те, що різати ТРЕБА, хоч дієслова в переліку немає
    cut = [
        ("— Ми виживали як могли, — ділиться спогадами Давид Арахамія.",
         "Давид Арахамія"),
        ("— Це вже не смішно, — кидає в залу депутатка Аліна Квітко.",
         "Аліна Квітко"),
        ("— Гроші знайдуться, — підхоплює його заступник Віталій Луков.",
         "Віталій Луков"),
        ("— Ніхто не перевіряв сім років, — обурюється мешканка будинку.", ""),
    ]
    for raw, want_who in cut:
        speech, who = carousel.split_speech(raw)
        check(f"ріже незнайоме дієслово: {raw[:34]}…",
              "—" not in speech and who == want_who, f"{speech!r} / {who!r}")

    # ---------- 13. Будова підпису під постом ----------
    print("\n— Підпис починається заголовком —")
    draft = carousel.fake_plan(a739)["post_caption"]
    check("чернетка підпису починається заголовком статті",
          draft.startswith(a739["title"]), draft[:70])
    check("а далі йде деталь із ліда", len(draft) > len(a739["title"]) + 20,
          f"{len(draft)}")
    check("такий підпис /stat знайде сам",
          carousel.caption_reach(draft, a739)["verdict"] == "ok",
          str(carousel.caption_reach(draft, a739)))
    check("підпис БЕЗ заголовка сам не знаходиться",
          carousel.caption_reach(
              "Сталася дивна історія, люди обурені, а відповідальні мовчать.",
              a739)["verdict"] != "ok")
    check("коли й так знайдеться, слів не радимо",
          not carousel.caption_reach(draft, a739)["missing"])

    check("«посилання в шапці профілю» зрізається кодом",
          carousel.clean_caption(
              "Суть новини.\n\nДеталі — за посиланням у шапці профілю.")
          == "Суть новини.")
    check("і «читайте на сайті» теж",
          carousel.clean_caption("Суть новини.\n\nЧитайте повний текст на сайті.")
          == "Суть новини.")
    check("а звичайне посилання на джерело лишається",
          "запит" in carousel.clean_caption(
              "Про це йдеться у відповіді на запит МикВісті."))
