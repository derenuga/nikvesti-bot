...
def test_canon_warning():
    ...
    check("картка показує чужих носіїв короткого написання",
          "У короткого написання ще" in txt_bw
          and "Олег Демченко (3)" in txt_bw
          and "Олександр Сєнкевич" not in txt_bw.split("У короткого")[1]
          .split("\n")[0],
          txt_bw)
    check("і рахує їхню частку від згадок короткого написання",
          "6%" in txt_bw.split("У короткого")[1].split("\n")[0], txt_bw)
    # Коли в голої форми чужих носіїв немає — попередження зайве («міністр
    # фінансів» ~ «міністр фінансів України» це та сама людина і той самий
    # орган, і зайвий крик там гасив би увагу до справжніх випадків).
    clean = {**bare_wide,
             "b": {**bare_wide["b"], "people": 1,
                   "names": ["Олександр Сєнкевич"],
                   "by_name": {"Олександр Сєнкевич": 76}}}
    check("а без чужих носіїв попередження немає",
          "У короткого написання ще" not in er._question_text(clean))
    # Клас не про вкладеність — рядок не з'являється взагалі.
    check("рядок належить лише класам «уточнення»",
          "У короткого написання ще" not in er._question_text(
              dict(bare_wide, cls="typo")))
