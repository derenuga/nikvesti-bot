...
def test_outliers():
    ...
    d, found = er.role_near_name(hist, "Володимир Чайка", "мер миколаєва")
    check("роль ПОРУЧ з іменем → попередник, не помилка",
    ...
    d2, found2 = er.role_near_name(wrong, "Віталій Кім", "мер миколаєва")
    check("роль ДАЛЕКО від імені → підозра на помилку витягу",
          found2 and d2 is not None and d2 > er.NEAR_CHARS, f"дистанція={d2}")

    d3, found3 = er.role_near_name("Текст без цієї людини взагалі.",
                                   "Віталій Кім", "мер миколаєва")
    check("імені немає в тексті — теж підозра, і це видно окремо",
          not found3 and d3 is None, f"{d3}/{found3}")

    d4, _ = er.role_near_name("Мером Миколаєва тоді був Володимир Чайка.",
