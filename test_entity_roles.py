...
def test_canon_warning():
    ...
    # Двоє чужих носіїв по ОДНІЙ згадці — це викид витягу, а не другий носій
    # посади: реальний випадок «директор зоопарку» ~ «директор Миколаївського
    # зоопарку», де до Топчого (16 і 28) причепились двоє по одному разу.
    # Картка мусить відрізняти це від дев'яти мерів інших міст, інакше обидві
    # ситуації виглядають однаково і людина зупиняється на кожній.
    zoo = {**bare_wide,
           "b": {**bare_wide["b"], "links": 18, "people": 3,
                 "by_name": {"Олександр Сєнкевич": 16, "Ігор Беляков": 1,
                             "Кирил Трантін": 1}}}
    txt_zoo = er._question_text(zoo)
    check("двоє чужих по одній згадці підписані як викид витягу",
          "викид витягу" in txt_zoo and "Ігор Беляков (1)" in txt_zoo, txt_zoo)
    check("а справжні чужі носії такої приписки не отримують",
          "викид витягу" not in txt_bw, txt_bw)
