...
CHECKS = {
    320092: [
        ...
        # Зона перевірки (рішення Олега 22.08): громади області ми більше не
        # перевіряємо, АЛЕ обіцянку ОВА перевіряємо однаково, кому б вона не
        # була дана. Коблеве тут — предмет, Кім — голова ОВА, і це рівно та
        # пара, на якій найлегше прочитати «область → не місто → в тінь».
        ("обіцянка ОВА про громаду області не ховається",
         lambda its: all(it.get("scope") != "local" for it in its),
         "scope=local сховав би заяву голови ОВА — а область лишається "
         "нашою зоною перевірки, кому б вона не обіцяла"),
    ],
    294413: [
    ...
    312757: [
        ...
        # Найдорожча помилка осі kind, спіймана на живому банку 17.08: правило
        # «offtopic — те, чого редакція не перевіряє» модель прочитала як
        # «обіцяльник не мерія», і дотація місту 1,18 млрд із держбюджету
        # пішла в тінь. Тут та сама конструкція: КМУ обіцяє гроші
        # миколаївському ліцею.
        ("обіцянка держави МІСТУ не названа чужою",
         lambda its: not _has(its, kind="offtopic")
                     and any(it.get("kind") == "commitment"
                             and ("кабінет" in _text(it, "promiser").lower()
                                  or "кму" in _text(it, "quote").lower()
                                  or "держав" in _text(it, "title").lower())
                             for it in its),
         "offtopic вирішує ПРЕДМЕТ, а не ранг обіцяльника: Кабмін, що обіцяє "
         "фінансування миколаївському ліцею, — наша обіцянка"),
    ],
    322324: [
        ("вимогу суду не викинуто як «прохання»",
         lambda its: len(its) >= 1,
         "окрему ухвалу адресат проігнорувати не може — це зобов'язання, "
         "хай і накладене ззовні. Правило про прохання (17.08) не має "
         "з'їдати обов'язкові документи"),
        ("адресата не зроблено обіцяльником, який щось пообіцяв",
         lambda its: not any(it.get("modality") in ("promised", "guaranteed")
                             for it in its),
         "«Відомство просять провести аудит» — Міноборони саме нічого не "
         "обіцяло; модальність тут planned, інакше реєстр припише йому "
         "мовленнєвий акт, якого не було"),
        ("підстава-документ названа",
         lambda its: any(_text(it, "based_on_document") for it in its),
         "без окремої ухвали запис висить у повітрі: саме документ і робить "
         "вимогу зобов'язанням"),
        # Рішення Олега 22.08, на цій самій статті: «це не обіцянка, це
        # всього лише звернення». Запис лишається (справу редакція розкопала
        # сама, і ухвала посилається на її публікацію 2019 року), але в чергу
        # журналіста не йде — обов'язок, накладений ЗЗОВНІ, не є обіцянкою
        # влади. Механізм той самий, що для процедурних кроків: kind.
        ("обов'язок ззовні пішов у тінь, а не в чергу",
         lambda its: _all(its, kind="process"),
         "суд зобов'язав Кабмін — це не обіцянка Кабміну; запис лишається в "
         "банку для пошуку й картки, але редакція не отримує його в чергу"),
        ("одна ухвала не розсипалась на список вимог",
         lambda its: len(its) <= 3,
         "ухвала до Кабміну й ухвала до Міноборони — два адресати, отже "
         "щонайбільше два-три записи. У банку з цієї статті лежало ШІСТЬ: "
         "аудит · припинення · стягнення · притягнення винних · порядок "
         "передачі · податок — тобто перелік вимог одного документа"),
        # Друга діра осі `scope`, і саме ця стаття її показала. Тут ні
        # міського предмета, ні обласного обіцяльника: київський суддя пише
        # Кабміну про землі полігонів НА МИКОЛАЇВЩИНІ. За наївним правилом
        # «усе інше — local» найбільша земельна історія року пішла б у тінь.
        ("предмет-область тримає запис видимим",
         lambda its: all(it.get("scope") != "local" for it in its),
         "Миколаївщина в цілому — наша зона перевірки незалежно від того, "
         "хто обіцяє: scope=local тут ховає 4,2 тисячі гектарів земель "
         "оборони, бо обіцяльник виявився не обласним"),
    ],
}
...
CASE_IDS = [294413, 320092, 311271, 320276, 317853, 321833, 312757, 322324]
...
CASE_WHY = {
    ...
    322324: "обов'язок, накладений ЗЗОВНІ: окрема ухвала суду; «просять» ≠ «пообіцяли»; перелік вимог одного документа — не список обіцянок",
}
...
REFERENCE = {
    294413: [
        {
            ...
            "kind": "commitment", "micro": False, "scope": "city",
    ...
    320092: [
        {
            ...
            "based_on_document": None, "amount": None,
            "kind": "rhetoric", "micro": False, "scope": "oblast",
    ...
    311271: [
        ...
        },
        {
            ...
            "actor_hidden": False, "framed_as_promise": False,
            "based_on_document": None, "amount": None,
            "kind": "rhetoric", "micro": False, "scope": "city",
    ...
    320276: [
        ...
        },
        {
            ...
            "kind": "process", "micro": False, "scope": "city",
    ...
    322324: [
        {
            "title": "Врегулювати передачу земель оборони для господарського використання",
            "subject": "землі оборони на Миколаївщині",
            "objects": ["полігон «Широкий Лан»", "Києво-Олександрівський авіаполігон"],
            "promiser": "Кабінет Міністрів України", "promiser_role": None,
            "owner": "Кабінет Міністрів України", "reported_by": "Роман Бойко",
            "audience": "community", "polarity": "do", "modality": "planned",
            "source_type": "government_decision",
            "deadline": None, "deadline_precision": None,
            "criterion": "порядок використання земель оборони затверджено урядом",
            "verification_method": "document_request",
            "condition": None, "condition_self_judged": False, "trigger_event": None,
            "actor_hidden": True, "framed_as_promise": False,
            "based_on_document": "окрема ухвала судді Господарського суду Києва "
                                 "від 10 серпня у справі №910/8859/26",
            "amount": None,
            "kind": "process", "micro": False, "scope": "oblast",
            "quote": "Уряд має розробити та прийняти рішення щодо можливості передачі земель оборони для господарського використання, а також знайти комплексне рішення щодо земельного податку за користування такими землями в умовах воєнного стану.",
        },
        {
            "title": "Провести аудит земель оборони і припинити незаконне користування",
            "subject": "землі оборони на Миколаївщині",
            "objects": ["полігон «Широкий Лан»"],
            "promiser": "Міністерство оборони України", "promiser_role": None,
            "owner": "Міністерство оборони України", "reported_by": "Роман Бойко",
            "audience": "community", "polarity": "do", "modality": "planned",
            "source_type": "government_decision",
            "deadline": None, "deadline_precision": None,
            "criterion": "аудит проведено, незаконне користування припинено, "
                         "збитки стягнуто",
            "verification_method": "document_request",
            "condition": None, "condition_self_judged": False, "trigger_event": None,
            "actor_hidden": True, "framed_as_promise": False,
            "based_on_document": "окрема ухвала у справі №910/8859/26",
            "amount": None,
            # ОДИН запис на всю ухвалу до цього адресата, хоч вимог у ній
            # п'ять: аудит · припинення · звільнення земель · притягнення
            # винних · стягнення збитків. Саме тут витяг дав шість записів.
            "kind": "process", "micro": False, "scope": "oblast",
            "quote": "Відомство просять провести аудит та інвентаризацію земель оборони, які передавалися або зараз передані у користування, припинити незаконне користування ними та забезпечити звільнення таких земель, ініціювати притягнення винних осіб до відповідальності, а також забезпечити стягнення завданих збитків і недоотриманих Міноборони доходів.",
        },
    ],
}
