...
def test_dupes_and_merge():
    ...
    try:
        with conn.cursor() as cur:
            ...
            check("…і сума замість слова «кошти» теж (найчастіше формулювання)",
                  pp._title_is_step("Виділити 60 млн гривень на харчування у "
                                    "дитсадках до жовтня"))
            check("організація самої послуги кроком не є",
                  not pp._title_is_step("Організувати харчування у дитячих "
                                        "садках до жовтня 2026 року"))
            check("гроші програють результату, який можна перевірити",
                  pp.merge_winner(
                      {"id": 806, "revisions": 5,
                       "title": "Виділити 60 млн гривень на харчування у "
                                "дитсадках до жовтня"},
                      {"id": 1791, "revisions": 1,
                       "title": "Організувати харчування у дитячих садках до "
                                "жовтня 2026 року"})[0]["id"] == 1791,
                  "довший ланцюг переїжджає до переможця, тому ревізії тут "
                  "не аргумент")
            ...
            # ТРАНШІ. Та сама дія над тим самим об'єктом, але з різними
            # строками — це етапи одного рішення, а не дубль: «Безпечне
            # місто» це 24 точки у 2026, 8 у 2027 і 8 у 2028, і злиття
            # з'їло б саме ті транші, які найлегше не виконати.
            check("однакова дія з РІЗНИМИ строками — транші, а не дубль",
                  not pjg.group_survives({"same_action": True,
                                          "doable_separately": False,
                                          "stage_of_same_result": False,
                                          "same_object": True},
                                         [1798700000, 1830200000]))
            check("…а стадії-результату різні строки законні",
                  pjg.group_survives({"same_action": False,
                                      "doable_separately": True,
                                      "stage_of_same_result": True,
                                      "same_object": True},
                                     [1798700000, 1830200000]),
                  "ПКД до березня, сам об'єкт до грудня")
            ...
            # Повний список їде ФАЙЛОМ — числа в чат, деталі у файл (те саме
            # правило, що /roles_audit і /entity_junk). Рішення ухвалюють за
            # цитатою й новиною, а на екрані їх не було взагалі.
            ordered = [
                ({"a": 11, "b": 12, "sim": 0.37, "deadline": None,
                  "judged_same": True},
                 {"id": 12, "title": "Облаштувати центр"},
                 {"id": 11, "title": "Розробити ПКД центру"}),
                ({"a": 21, "b": 22, "sim": 0.29, "deadline": None,
                  "judged_same": False},
                 {"id": 21, "title": "Перша"}, {"id": 22, "title": "Друга"}),
            ]
            revs_f = {12: {"quote": "тут буде центр", "article_id": 900},
                      11: {"quote": "спершу документація", "article_id": 900},
                      21: {"quote": "щось", "article_id": 901}}
            links_f = {900: {"title": "Новина", "url": "https://x/900"},
                       901: {"title": "Інша", "url": "https://x/901"}}
            doc = ph._dupes_file(ordered, revs_f, links_f,
                                 {(11, 12): {"why": "стадія і результат"}})
            check("у файлі видно, ЩО лишиться і ЩО приєднається",
                  "1. ЛИШИТЬСЯ: Облаштувати центр" in doc
                  and "ПРИЄДНАЄТЬСЯ: Розробити ПКД центру" in doc, doc[:200])
            check("у файлі є цитати обох — без них рішення неможливе",
                  "тут буде центр" in doc and "спершу документація" in doc)
            check("спільна новина друкується ОДИН раз, а не двічі",
                  doc.count("https://x/900") == 1, doc)
            check("підстава судді в файлі є", "стадія і результат" in doc)
            check("непідтверджені йдуть окремим блоком із готовою командою",
                  "ПРОСТО СХОЖІ НАЗВИ: 1" in doc
                  and "/promise_merge 21 22" in doc)
            check("нумерація файлу збігається з номерами кнопок",
                  doc.index("1. ЛИШИТЬСЯ") < doc.index("ПРОСТО СХОЖІ"))
...
def test_export_and_fix_packet():
    ...
    # Відкат автоматичних закриттів пакетом: рішень бота накопичується
    # десятками, і тапати /promise_reopen по одному — та сама робота, від
    # якої пакети й рятують. Вивантаження /promise_auto віддає готові рядки,
    # тож правити файл означає лише викинути зайве.
    back, berr = ph.parse_fix("# promises-fix\n# [ЗІРВАНО] Розширити програму\n"
                              "reopen 543\n/promise_reopen 2219\n")
    check("пакет розуміє reopen — і командним рядком теж",
          back == [("reopen", 543, None, None), ("reopen", 2219, None, None)],
          str(back))
    check("коментарі вивантаження не заважають", berr == [], str(berr))
...
def test_fulfil_detector():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Заяви без предмета («просувати інтереси області у держбюджеті»)
            # — магніт для хибних закриттів: будь-яка добра новина про
            # область виглядає їх підтвердженням. У прогоні 04.08 обіцянка 956
            # закрилась ТРИЧІ трьома різними новинами. Рішення про такі —
            # тільки людське, і в черзі вони саме тому в класі «перевірити
            # нічим».
            check("неперевірна обіцянка автоматично не закривається",
                  pf.unfalsifiable({"verifiability": "unfalsifiable"}))
            check("вимірювана — закривається",
                  not pf.unfalsifiable({"verifiability": "measurable"}))
...
def test_polarity_suspect():
    """Перевернута полярність: `not_do` там, де пообіцяли ДІЯТИ.

    Замір 22.08.2026 на живому банку: з 11 записів `not_do` перевернутих
    ВІСІМ. Механізм один — модель читає негативне ЗВУЧАННЯ результату
    («припинити», «мораторій», «заборона», «відключити») замість ФОРМИ
    зобовʼязання. Ціна: `not_do` за побудовою не буває простроченим, тож усі
    вісім не прозвонять ніколи (а в трьох є справжній строк), і логіка
    перевірки в них інвертована — виконання читається як зрив.

    Тест ганяє детектор на ВСІХ одинадцяти реальних заголовках: три справжні
    й вісім перевернутих. Критерій як сигнал тут свідомо не використано —
    у 663 він містить «люди НЕ можуть пройти», тобто заперечення є, а
    полярність усе одно перевернута.
    """
    genuine = [
        "Не випалювати суху рослинність в області",
        "Не демонтувати «лежачі поліцейські» по місту",
        "Утримати від скорочення працівників пологового та дитячого відділень",
    ]
    inverted = [
        "Відключити фонтан у сквері Металургів",
        "Припинити набір учнів до 10 класів у Баштанських ліцеях №1 та №2",
        "Закрити проходи до моря в Коблевому",
        "Підготувати проєкт рішення сесії про відмову визнавати ділянки",
        "Припинити самовільну реставрацію фасаду кафедрального собору",
        "Запровадити мораторій на омолоджувальну обрізку дерев",
        "Припинити газопостачання у селах Калинівка, Славне та Новоолександрівка",
        "Запровадити нічну заборону на стоянку автомобілів",
    ]
    miss = [t for t in genuine
            if pp.polarity_suspect({"title": t, "polarity": "not_do"})]
    check("справжнє «не робити» детектор не чіпає", not miss, str(miss))
    slip = [t for t in inverted
            if not pp.polarity_suspect({"title": t, "polarity": "not_do"})]
    check("усі вісім перевернутих упіймано", not slip, str(slip))
    check("записи do детектор не дивиться взагалі",
          not pp.polarity_suspect({"title": "Відремонтувати дорогу",
                                   "polarity": "do"}))
    check("порожній заголовок не валить детектор",
          pp.polarity_suspect({"title": None, "polarity": "not_do"}) is True)

    # Друга половина ціни: клас черги. `not_do` чекає події й не
    # прострочується — саме тому перевернутий запис мовчить назавжди.
    now = int(time.time())
    row = {"polarity": "not_do", "deadline": now - 30 * DAY, "status": "expected",
           "verifiability": "event_triggered", "condition": None,
           "modality": "promised", "trigger_event": None}
    check("«не робити» зі старим строком не стає простроченим",
          pp.queue_class(row, now) != "overdue", pp.queue_class(row, now))
    check("а те саме з полярністю «зробити» — прострочене",
          pp.queue_class(dict(row, polarity="do", verifiability="measurable"),
                         now) == "overdue")
