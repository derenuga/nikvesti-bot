...
--
-- ЯК ЗАПУСТИТИ (два кроки, обидва неочевидні — обидва пройдені 16.08):
--
--   1. Railway → сервіс Postgres → вкладка **Console**. Це НЕ консоль бази,
--      а shell усередині контейнера: SQL, вставлений туди, зустрічає bash і
--      відповідає «DO: command not found». Спершу зайти в базу:
--          psql "$DATABASE_URL"        (запасний варіант: psql -U postgres railway)
--      Ознака успіху — запрошення змінилось на `railway=#`.
--   2. Замінити пароль, вставити файл, виконати. Вихід — \q.
--      Повторний запуск безпечний: роль не дублюється, пароль оновиться.
--
-- НЕ у вкладці Database → Data: поле з підказкою «SELECT * FROM sync_state»
-- це переглядач таблиць, він дописує власний LIMIT і падає з «syntax error
-- at or near LIMIT» одразу після END $$.
--
-- Якщо веб-термінал корчить довгу вставку (рядки наїжджають один на одного —
-- теж траплялось), є короткий варіант без DO-блока: CREATE ROLE + три GRANT
-- одним списком таблиць. Він у README сесії; цей файл лишається повним, бо
-- пояснює МЕЖУ доступу, а не лише виконує її.
--
