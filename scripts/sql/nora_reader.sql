...
--
-- ЯК ЗАПУСТИТИ: Railway → сервіс Postgres → вкладка Database → поле запиту
-- (те, у якому сірим написано «SELECT * FROM sync_state»). Замінити пароль у
-- третьому рядку, вставити ВЕСЬ файл, виконати. Це ОДИН оператор — саме тому
-- він обгорнутий у DO-блок: віконця в панелі не люблять пачку команд.
-- Повторний запуск безпечний: роль не дублюється, пароль просто оновиться.
--
