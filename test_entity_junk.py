...
def test_positions():
    ...
    check("посада без ролі в норі ловиться першим словом",
          ...
          hit2["signals"])
    check("сигнал лише за словом підписаний як СЛАБКИЙ "
          "(на ньому спливали «Королівська канадська кінна поліція» і «Лідер»)",
          "СЛАБКИЙ" in hit2["signals"] and "СЛАБКИЙ" not in hit["signals"],
          hit2["signals"])
...
def test_org_form_key():
    ...
    check("прикметник перед формою теж зрізається (порядок патернів)",
          ep.org_key("Міське комунальне підприємство «Миколаївводоканал»")
          == ep.org_key("КП «Миколаївводоканал»") == "миколаївводоканал")
    check("родові слова «компанія / група компаній / фірма» — теж форма",
          ep.org_key("Група компаній «Автострада»")
          == ep.org_key("Компанія «Автострада»")
          == ep.org_key("Автострада") == "автострада")
    check("але «асоціація» і «фонд» — частина назви, не форма",
          ep.org_key("Асоціація міст України") == "асоціація міст україни")
    check("приватне підприємство — теж форма",
          ep.org_key("приватне підприємство «ІНАУТ»") == "інаут")
...
def test_org_dupes():
    ...
    # Зайве слово, яке НЕ є нашим топонімом, робить іншу установу: «ЗМІ
    # «МикВісті»» — випадкове означення в одній статті, «Європейський
    # БОКСЕРСЬКИЙ союз» — узагалі не Євросоюз. Такі пари в кандидати не йдуть.
    bot_db.execute("INSERT INTO entities (id, kind, name_ua, mentions) VALUES "
                   "(7031, 'org', 'МикВісті', 1543), "
                   "(7032, 'org', 'ЗМІ «МикВісті»', 2), "
                   "(7033, 'org', 'МикВісті України', 2), "
                   "(7034, 'org', 'Європейський Союз', 990), "
                   "(7035, 'org', 'Європейський боксерський союз', 3)")
    # Поріг нижчий за прод-дефолт: «МикВісті» ~ «МикВісті України» дають
    # trgm-схожість 0.53, і на 0.55 пара просто не дійшла б до класифікації —
    # а перевіряємо ми тут саме напрямок, а не поріг.
    pairs2, _sk = ej.find_org_dupes(threshold=0.4, limit=50)
    got = {frozenset((p["winner"][0], p["loser"][0])) for p in pairs2}
    check("випадкове означення в кандидати не потрапляє",
          frozenset((7031, 7032)) not in got, str([tuple(x) for x in got]))
    check("боксерський союз не пропонується Євросоюзу",
          frozenset((7034, 7035)) not in got, str([tuple(x) for x in got]))
    # …а там, де зайве слово — НАША країна, пара лишається кандидатом, і
    # напрямок вирішує вжиток: 1543 проти 2 переважують повноту назви.
    hit = [p for p in pairs2 if {p["winner"][0], p["loser"][0]} == {7031, 7033}]
    check("при разючій різниці у вжитку лишається ВЖИВАНІША картка",
          hit and hit[0]["winner"][0] == 7031,
          str([(p["winner"][0], p["loser"][0]) for p in pairs2]))
    bot_db.execute("DELETE FROM entities WHERE id >= 7031 AND id <= 7035")
