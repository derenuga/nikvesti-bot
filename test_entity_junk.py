...
def test_org_dupes():
    ...
    # …АЛЕ повнота програє разючій різниці у вжитку: «ЗМІ «МикВісті»» (2) не
    # повніша офіційна назва «МикВісті» (1543), а випадкове означення в одній
    # статті. Живий випадок 02.08: список пропонував саме такий напрямок.
    bot_db.execute("INSERT INTO entities (id, kind, name_ua, mentions) VALUES "
                   "(7031, 'org', 'МикВісті', 1543), "
                   "(7032, 'org', 'ЗМІ «МикВісті»', 2)")
    pairs2, _sk = ej.find_org_dupes()
    hit = [p for p in pairs2 if {p["winner"][0], p["loser"][0]} == {7031, 7032}]
    ...
    bot_db.execute("DELETE FROM entities WHERE id = ANY(%s)", ([7031, 7032],))
