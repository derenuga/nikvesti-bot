...
- Прозорро — окремого дока немає, детальний опис у док-стрінгу `handlers/prozorro.py` (API, офсети, продуктивність, паралельні запити)
...
- /prozorro, /prozorro_test_jump, /prozorro_confirm_jump, /prozorro_reset_tender — див. док-стрінг handlers/prozorro.py
...
> Апрувнутий план ревізії липня 2026 (оптимізації, AI-шар, нові модулі, порядок хвиль) — окремо в `REVIEW_2026_07.md`. Нижче — старий операційний беклог; нумерацію не міняти (на пункти посилається LONG_TERM_VISION.md).

1. **AI рефакторинг** — ✅ ЧАСТКОВО / переформульовано: prompt caching для NLQ-роутера і єдиний AI-хелпер — це пункти б.2 і в.1 в REVIEW_2026_07.md (хвиля 2). Виносити короткий FOX_SYSTEM_PROMPT у кеш окремо — сенсу немає (~700 символів).

2. **Конкуренти — підводка AI** — ✅ ЗРОБЛЕНО (07.2026): generate_competitors_intro підключена в check_competitors.

3. **Конкуренти — novosti-n.org** — ✅ ЗРОБЛЕНО: друге джерело в competitors.py.
...
7. **Документи — проєкти рішень міської ради** — ✅ ЗРОБЛЕНО: джерело mkrada_decisions (content/proekti-rishen-miskradi.html, окремий парсер).

8. **Документи — проєкти рішень виконкому** (c=5) — є PDF на окремих сторінках документів, потрібен додатковий запит для посилання.

9. **Правоохоронці — розширення джерел** — поліція (ГУНП, mk.npu.gov.ua) ✅ додана; лишились СБУ, ДСНС.
...
17. **Перенести бота на KEY4** — зараз на Railway; Railway не підтримує статичний IP для whitelist зовнішньої БД.
...
21. **Іра Федорович в TEAM** — ✅ ЗРОБЛЕНО: є в словнику TEAM у ai_messages.py.

---

**Довгострокове бачення (інфраструктура, монетизація, AI над архівом за 17 років) — окремий документ:** `LONG_TERM_VISION.md`. Пункти 11, 12, 17 вище — операційні передумови для нього.
