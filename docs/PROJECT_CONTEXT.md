...
22. **Бекфіл статей у нору** — з 30.07 нора дзеркалить і type='article' (щогодинний інкремент підхоплює нові/оновлені), але ІСТОРИЧНІ статті в нору не залиті: курсор бекфілу їх уже проминув, коли фільтр був type='news'. Треба разовий прогін по nodes WHERE type='article' (їх небагато проти 200к новин). До того статті в FTS/досьє/беках невидимі; імпакт-архів працює через фолбек у БД сайту. Точково — /nora_resync <id>. Бриф для окремої сесії — docs/ARTICLES_BACKFILL_BRIEF.md.
