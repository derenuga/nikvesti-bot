...
13. ~~**Звіти по співробітниках** — скільки новин опублікував кожен автор.~~ ✅ Зроблено 27.07.2026 у Mini App «Команда»: KPI-норми з авто-фактом із nodes, дашборд виконання по періодах і помісячна динаміка людини (TEAM_APP_MODULE.md). Плюс NLQ-tool count_news для разових питань.
...
21. **Порадник щодо зарплат і премій** — інструмент менеджеру: кому підвищувати, кому премію, спираючись і на виконання, і на якість. Задум із дослідженням (Кон, Десі/Раян, Кемпбелл, Герцберг, Адамс, stack ranking Microsoft) — [`COMPENSATION_ADVISOR.md`](COMPENSATION_ADVISOR.md). Потребує двох джерел, яких ще немає: журналу впливу (в роботі) і квартальної оцінки редактора.

22. ~~**Бекфіл статей у нору**~~ — ✅ зроблено 30.07: команда `/articles_backfill [N]` (archive_mirror.py) — разовий resumable-прогін по nodes WHERE type='article' зі своїм курсором `articles_backfill_last_id` (основні курсори дзеркала не чіпає), зупинка — /archive_stop. Лишилось запустити на проді й звірити: /nora_sql SELECT kind, COUNT(*) FROM articles GROUP BY kind; маркер — стаття 311397.

23. **Іра Федорович в TEAM** — додати `{"tg": "@diiessa", "role": "перекладачка, англійська версія сайту", "birthday": "24.05"}` в словник TEAM у ai_messages.py.
