...
27. Стисла переспроба замість заглушки «надто довго» (07.2026): важкі порівняння (напр. «тиждень до тижня без Google Discover») з'їдали 4096-токенний бюджет на adaptive-thinking + арифметику по днях і обривались на `max_tokens` з порожнім текстом — код спихав на користувача «звузь період». Тепер при `max_tokens` без тексту робиться один прохід із піднятою стелею (8192) і жорсткою вимогою стислості: усі дані вже в `messages`, переспроба лише формулює короткий підсумок (6-8 рядків, ключові числа + висновок). Плюс підказка в system: питання «трафік без каналу X» брати підсумками ПЕРІОДУ по `sessionDefaultChannelGroup` (один виклик на період, без денної розбивки), а не віднімати по днях; Discover у GA4 окремим каналом не виділяється — чесно позначати оцінку.
