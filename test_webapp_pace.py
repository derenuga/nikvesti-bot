...
- у середині місяця з відставанням у фразі стоять реальні числа «факт із
  очікуваного», а колір — бурштиновий, НЕ червоний: на власному екрані червоне
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            check("у фразі немає слова про відставання", "позаду" not in say)
            check("названо, скільки робочих днів попереду", "21" in say)
        ...
        try:
            await page.wait_for_selector(".pace-say", timeout=5000)
            say = await page.inner_text(".pace-say")
            check("у фразі стоїть факт", "7" in say)
            check("і очікування на сьогодні", "9" in say)
            check("сказано, скільки днів попереду", "12" in say)
