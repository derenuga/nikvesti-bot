...
- при відставанні кільце СИНЄ — ні червоне, ні бурштинове. Бурштин тут стояв
  із 28.07 як «підтягнись», і Олег скасував його 11.08: своє фото в жовтому
  кільці читається як вирок, хоч би що колір означав у коді, а темп і без
  нього сказаний двічі — засічкою «де ти маєш бути сьогодні» і фразою «Час
  додати темпу». Шкала червоний→жовтий→зелений лишилась менеджерському
  «Звіту»; на власному екрані зеленіє лише ЗАКРИТА норма;
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            # Олег, 11.08: кільце людини на ЇЇ екрані не оцінює її кольором —
            # воно синє і зеленіє лише на закритій нормі. Стережемо обидва
            # боки: що синє І що жодного кольору тривоги там немає.
            ring = await page.locator(".me-cap b").get_attribute("style") or ""
            check("відставання синє — людину кольором не судять",
                  "--blue" in ring)
            check("ні червоного, ні бурштинового на власному кільці",
                  "--bad" not in ring and "--warn" not in ring)
