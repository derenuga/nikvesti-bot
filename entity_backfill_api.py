#!/usr/bin/env python3
"""Бэкфіл сутнісного шару через Anthropic Batch API — ОСНОВНИЙ канал (§3.3.1).

Рішення 02.08.2026: женемо Haiku 4.5 через Message Batches API (−50% до ціни).
Альтернатива (суб-агенти на підписці Max) виміряна і відхилена: гірша якість
при більшому розході токенів — числа в §3.3.1 плану.

Дизайн (він і дає перевагу в якості над пачками по 25 статей):
...
Команди (з бота — /entity_estimate і /entity_backfill з тими самими аргументами):
    python3 entity_backfill_api.py count 2022-01-01 2027-01-01
        ...
        Третій аргумент fetch_range — тип (article|news|None): статей на
        порядок менше за новини, тож догнати спершу їх коштує пару доларів
        замість сотень за роки новин.
...
# Кап виводу. Історія — двічі та сама граблина, тож тут запас навмисне великий:
#   2000 → 3000  після першого прогону (139 непарсибельних із 27.9к): статті про
#                відключення світла — обленерго плюс десятки вулиць — не влазили;
#   3000 → 8000  інцидент 02.08: витяг падав на JSONDecodeError «Unterminated
#                string ... char 7398» — JSON обривався рівно на капі. Обидва
#                рази піднімали замало.
# Платимо за ФАКТИЧНИЙ вихід (~550 токенів на статтю), стеля Haiku 4.5 — 64к,
# тож запас не коштує нічого. Кап тут лише запобіжник від нескінченної
# генерації, а не інструмент економії — не опускати «щоб дешевше».
MAX_TOKENS = 8000
