...
def setup_scheduler(bot, last_channel_post_time=None):
    ...
    # Інкремент банку тем — ДВІЧІ НА ДОБУ, 06:25 і 18:25 (опт-ін
    # /promise_increment_on). Був щогодинним, і саме ритм коштував грошей:
    # інструкція витягу — 10 300 токенів, стаття — 850, тобто на кожен прогін
    # ми заново оплачували незмінний промпт заради однієї-двох статей. Кеш є,
    # але живе 5 хвилин і до наступної години завжди мертвий, тож із нього
    # майже не читали. Два прогони — це ~11 статей на раз: перша пише кеш,
    # решта читають його вдесятеро дешевше ($5.3 → $1.4 на місяць).
    #
    # Оперативність тут не потрібна (рішення Олега, 17.08): нагадування в
    # канал ідуть о 09:10 раз на добу, а детектор виконання дивиться вікно в
    # сім днів. Втратити нічого не можна за побудовою — відбір статей іде за
    # ФАКТОМ відсутності розбору, а не курсором, тож пропущена стаття просто
    # чекає наступного прогону (INCR_MAX_PER_RUN = 60 проти ~22 статей на
    # добу, пік 34 — запас майже вчетверо).
    #
    # Час обрано так, щоб ранковий прогін устиг ДО нагадувань о 09:10, а
    # вечірній забрав робочий день; обидва — після сутнісного шару (:55),
    # бо витяг резолвить предмет і обіцяльника в картки.
    scheduler.add_job(sync_promises_incremental, "cron", hour="6,18", minute=25,
                      args=[bot])
    ...
    # Детектор виконання — ДВІЧІ НА ДОБУ, 06:40 і 18:40, одразу за витягом
    # (:25) і сутностями (:55): збіг шукається по КАРТЦІ сутності, тож свіжа
    # стаття має бути вже розібрана сутнісним шаром.
    #
    # ГРОШЕЙ САМ ПО СОБІ ЦЕЙ ПЕРЕНОС НЕ ЕКОНОМИТЬ, і це варто знати: пар за
    # добу стільки ж, судити їх однаково доводиться кожну, а вердикт по парі
    # пишеться назавжди (promise_closures), тож щогодинний прогін не платив
    # двічі. Ціну детектора зняв пре-фільтр (див. CONTAINER_PLACE_* у
    # promise_pipeline) — 2287 пар за серпень проти ~800 після нього. Тут же
    # причина інша: одна справа замість двадцяти чотирьох на добу, менше шуму
    # в логах і той самий ритм, що у витягу.
    #
    # Стелю MAX_PAIRS піднято з 60 до 200 саме через це: на щогодинному
    # розкладі недобрана пара поверталась за годину, а тепер — за пів доби.
    scheduler.add_job(promise_fulfil_hourly, "cron", hour="6,18", minute=40,
                      args=[bot])
    # Зірочки — щогодини о :50. Лишились ЩОГОДИННИМИ, хоч витяг і детектор
    # переїхали на двічі на добу: тут немає жодного виклику моделі (звіряємо
    # лічильник ревізій зі знімком у зірочці), тобто ритм нічого не коштує, —
    # а ревізії з'являються не лише за розкладом: /promise_scan і
    # /promise_retest Олег ганяє руками будь-коли, і сигнал власнику зірочки
    # не має чекати вечора. Порядок усередині години збережено: :50 — після
    # витягу (:25) і детектора (:40), інакше людина отримала б сигнал про
    # подію, якої ще немає в картці.
