...
# в силі.
#
# СПИСОК ДІЄСЛІВ — ПРИСКОРЮВАЧ, А НЕ ПРАВИЛО (22.08). Спершу хвіст ловився
# ЛИШЕ за знайомим дієсловом, і це ламалось рівно так, як мусило: «згадує
# Давид Арахамія» (322739) у список не входило, тож ім'я поїхало і в цитату,
# і в підпис під нею — двічі на одному слайді. Дієслова мовлення в
# українській це ВІДКРИТИЙ клас (згадує, ділиться, резюмує, кидає, підхоплює),
# і будь-який їх перелік застаріває на наступній статті. Тому нижче хвіст
# розпізнається за БУДОВОЮ фрази, а список лишився швидким шляхом для
# випадків, які він і так покриває.
_SPEECH_VERB = (r"(?:сказа|зазнач|додав|додала|повідом|розпов|наголос|поясн|"
                ...
                r"запита|вигукн|перекон|іронізу|обурив|обурила|згаду|згада)")
_VERB_RE = re.compile(_SPEECH_VERB, re.IGNORECASE)

# Довший хвіст — уже не підпис, а частина думки
TAIL_MAX = 160

# Службові слова: ними починається ПРОДОВЖЕННЯ фрази, а не підпис. Клас
# закритий (їх у мові десятки і нових не з'являється) — саме тому тест на
# них надійніший за перелік дієслів, яких сотні й список ніколи не повний.
_GLUE_WORDS = set("""
і й а але та бо що щоб як коли тому оскільки адже проте однак або чи ні не
це цей ця ці цим цього той те ті я ми ти ви він вона воно вони
до від за на у в з із зі по при про для через над під між без після перед
хоча якщо аби ж же теж також лише тільки навіть вже ще саме отже тож тобто
там тут тоді потім зараз наразі просто майже мабуть звісно звичайно
""".split())

_LEAD_DASH_RE = re.compile(r"^\s*[—–]\s*")
_FIRST_WORD_RE = re.compile(r"\W*([^\W\d_][\w'’\-]*)", re.UNICODE)
...
def _cut_at_last_dash(text):
    """(мова, хвіст) по ОСТАННЬОМУ тире або None, якщо різати нема чого.

    Запобіжник один, зате обов'язковий: перед тире має щось лишитись. Цитата
    «— Так.» уся починається з тире, і воно ж останнє — без цієї перевірки
    від неї не лишилось би нічого.

    Порівнювати довжини мови й хвоста НЕ можна, хоч спокуса є: «— Ми виживали
    як могли, — ділиться спогадами Давид Арахамія» має хвіст ДОВШИЙ за саму
    мову, і таких коротких цитат у нас повно. Відсіює не довжина, а будова —
    див. _is_attribution."""
    i = max(text.rfind("—"), text.rfind("–"))
    if i <= 0:
        return None
    head, tail = text[:i].rstrip(), text[i + 1:].strip()
    if not head or not tail or len(tail) > TAIL_MAX:
        return None
    return head, tail


def _is_attribution(head, tail):
    """Чи цей хвіст — підпис «хто сказав», а не продовження думки.

    Три тести, кожен сам по собі достатній:
    1. знайоме дієслово мовлення — питань немає;
    2. мова закрилась «ялинкою», якою й відкривалась, — усе після неї вже не
       мова. Вимога, щоб лапки ОБРАМЛЯЛИ фразу, не випадкова: всередині цитат
       теж бувають назви в лапках («Зеленбуд»), і без неї «…відкриємо
       „Зеленбуд“, — і крапка» втратило б кінець думки;
    3. кома перед тире плюс перше слово хвоста не службове. Кома тут головна:
       українська кладе її перед підписом («…ногами, — згадує Давид»), а
       перед тире виділення — ні («Головне — люди»), тож саме вона відрізняє
       підпис від наголосу. Питальний чи окличний знак у кінці знімає
       підозру одразу: підписи так не закінчуються."""
    if _VERB_RE.search(tail):
        return True
    if tail.rstrip()[-1:] in "?!":
        return False
    m = _FIRST_WORD_RE.match(tail)
    if not m or m.group(1).lower() in _GLUE_WORDS:
        return False
    naked = head.rstrip(" ,;:")
    if head.lstrip().startswith(("«", "“", '"')) and naked.endswith(("»", "”", '"')):
        return True
    return head.endswith(",")


# Довше ім'я в підписі не буває: «Олександр Федорович Сєнкевич» це три слова
_WHO_MAX_WORDS = 4


def _who_from(tail):
    """Кого назвав хвіст — або порожньо, якщо нікого.

    Беремо ХВІСТ ІЗ СЛІВ З ВЕЛИКОЇ ЛІТЕРИ, бо в українському підписі ім'я
    стоїть останнім: «згадує Давид Арахамія», «як зазначив Сєнкевич»,
    «ділиться спогадами Давид Арахамія». Рахувати від початку не можна —
    дієслівна частина буває і з двох слів, і слайд підписався б «спогадами
    Давид Арахамія».

    «розповів політик» і «зазначається у відповіді» нікого не називають —
    великої літери в кінці немає, і підпису не буде: порожнє поле чесніше за
    слово «політик» під цитатою.

    Межа відома: у «каже мер Миколаєва Олександр Сєнкевич» разом з іменем
    приїде «Миколаєва» — топонім посади теж пишеться з великої. Живемо з цим
    свідомо: це ЗАПАСНИЙ шлях (коли агент повернув свій підпис, беруть його),
    і зайве слово в полі, яке людина бачить і править, дешевше за словник
    посад, який довелося б доповнювати так само, як список дієслів."""
    words = tail.strip(" .").split()
    name = []
    for word in reversed(words):
        if not _UPPER_RE.match(word[:1]) or len(name) >= _WHO_MAX_WORDS:
            break
        name.insert(0, word)
...
def split_speech(text):
    ...
    cut = _cut_at_last_dash(text)
    if cut and _is_attribution(*cut):
        text = cut[0].rstrip(" ,;:")
        who = _who_from(cut[1])
...
# Вигадана інструкція «куди йти». Ми не знаємо, що СММ поставить у профіль, і
# лінка в підписі Instagram однаково не буває — тож «деталі за посиланням у
# шапці профілю» це обіцянка від нашого імені, якої ніхто не давав (Олег,
# 22.08: «пусть не выдумывает такого»). Промпт про це каже, але просити мало —
# рядок зрізається кодом, як і хештеги.
_CTA_TAIL_RE = re.compile(
    r"(?:^|\n)[^\n]{0,120}?(?:посилання|лінк|link|силка)[^\n]{0,60}?"
    r"(?:шапц|профіл|біо|bio)[^\n]*|"
    r"(?:^|\n)[^\n]{0,120}?(?:читайте|детальніше|деталі|подробиц|більше)"
    r"[^\n]{0,60}?(?:на сайті|за посиланням|у нас на сайті)[^\n]*",
    re.IGNORECASE)


def clean_caption(text):
    """Підпис під пост: без хештегів, без вигаданих «лінк у шапці», без
    порожніх хвостів, у межах стелі."""
...
def fake_plan(article):
    ...
    # Та сама будова, що просимо в агента: заголовок першим рядком, деталь —
    # другим. Чернетка без моделі має виглядати як те, що модель віддала б,
    # інакше СММ звикає до одного вигляду й отримує інший
    title = (article.get("title") or "").strip()
    lead = (article.get("description") or "").strip()
    caption = "\n\n".join(p for p in (title, clip_words(lead, 240)) if p)
