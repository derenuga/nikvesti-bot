...
from handlers import bot_db, entity_pairs as epair, entity_roles
...
# ---------- Авто-злиття доведених класів (16.08.2026) ----------
#
# Навіщо. Розбір дублів руками закрив 281 картку — і це були найбільші
# картки нори. Але спосіб не масштабується: у базі 39 тисяч карток, і
# 1365 із них тримають 69% усіх згадок, тобто ручна робота ПРАВИЛЬНО
# цілилась у голову розподілу, а хвіст нею не закрити ніколи. Формулювання
# Олега: «у людини життя не вистачить, щоб усім цим займатись».
#
# Що можна віддати машині. Правило «кожну пару вирішує людина» написане для
# ЛЮДЕЙ, де буква міняє особу (Кличко / Клочко). Для організацій різниця
# синтаксична, і за день ми заміряли її на ~250 парах чотирьох класів
# (розкрита абревіатура, форма топоніма, порядок слів, правова форма) без
# жодної помилки. Ці класи й рахує ep.org_key — тобто групою тут є картки з
# ОДНАКОВИМ обчисленим ключем, і рішення в ній уже не додає інформації.
#
# Чого прохід НЕ робить:
#   • не чіпає людей і місця — лише kind='org';
#   • не чіпає пари, розведені людиною (entity_pairs);
#   • не чіпає короткі ключі: на 5–7 символах різні абревіатури збігаються
#     випадково, і виграш там нульовий, а ціна помилки та сама;
#   • нічого не вирішує сам щодо СУТІ — ключ не знає, що «Миколаївська
#     окружна прокуратура» районна, а не міська; він знає лише, що набір
#     слів різний. Саме тому «міста» лишилось значущим словом.
#
# Відкат — на весь прогін одразу: /entity_junk_undo <прогін>.
AUTODEDUP_MIN_KEY = 8


def autodedup_groups(kind="org"):
    """Групи карток з однаковим ключем зіставлення. Нічого не змінює."""
    ensure_schema()
    epair.ensure_schema()
    rows = bot_db.query(
        "SELECT id, kind, name_ua, name_ru, coalesce(mentions, 0) AS m "
        "FROM entities WHERE kind = %s", (kind,))
    buckets = {}
    for r in rows:
        for nm in (r["name_ua"], r["name_ru"]):
            ...
            if k and len(k) >= AUTODEDUP_MIN_KEY:
                ...
                break
    known = epair.load_different()
    groups, held = [], 0
    for k, members in buckets.items():
        if len(members) < 2:
            continue
        ids = [m["id"] for m in members]
        if any(epair.is_different(a, b, known)
               for i, a in enumerate(ids) for b in ids[i + 1:]):
            held += 1
            continue
        members.sort(key=lambda r: (-r["m"], -len(r["name_ua"] or ""), r["id"]))
        groups.append({"key": k, "winner": members[0], "losers": members[1:],
                       "mentions": sum(m["m"] for m in members)})
    groups.sort(key=lambda g: -g["mentions"])
    return groups, held


def autodedup_apply(groups, decided_by=None, run=None):
    """Виконати прохід. Кожне злиття окремо: збійне не валить решту."""
    run = run or f"auto-{int(time.time())}"
    done, failed = 0, []
    for g in groups:
        for l in g["losers"]:
            try:
                merge_cards(g["winner"]["id"], l["id"], decided_by, run)
                done += 1
            except Exception as e:
                failed.append(f"{l['id']} → {g['winner']['id']}: "
                              f"{type(e).__name__}: {e}")
    return {"run": run, "done": done, "failed": failed}


def autodedup_csv(groups):
    import csv
    import io
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["ключ", "лишається", "id", "згадки", "зникає", "id", "згадки"])
    for g in groups:
        for l in g["losers"]:
            w.writerow([g["key"], g["winner"]["name_ua"] or g["winner"]["name_ru"],
                        g["winner"]["id"], g["winner"]["m"],
                        l["name_ua"] or l["name_ru"], l["id"], l["m"]])
    return buf.getvalue().encode("utf-8-sig")


def _autodedup_text(groups, held):
    cards = sum(len(g["losers"]) for g in groups)
    lines = [f"🦊 Авто-злиття органів: {len(groups)} груп, {cards} карток "
             f"зникне, {sum(g['mentions'] for g in groups)} згадок зачеплено\n",
             "Групою тут є картки з ОДНАКОВИМ обчисленим ключем: різниця лише "
             "в абревіатурі, формі топоніма, порядку слів чи правовій формі. "
             "Пари, розведені раніше як різні, у прохід не йдуть"
             + (f" ({held} груп)" if held else "") + ".\n"]
    for g in groups[:12]:
        w = g["winner"]
        lines.append(f"«{w['name_ua'] or w['name_ru']}» ({w['m']}) ← "
                     + " · ".join(f"«{l['name_ua'] or l['name_ru']}» ({l['m']})"
                                  for l in g["losers"][:4])
                     + (f" …і ще {len(g['losers']) - 4}"
                        if len(g["losers"]) > 4 else ""))
    if len(groups) > 12:
        lines.append(f"\n…і ще {len(groups) - 12} груп — повний список файлом.")
    lines.append("\nПовний прохід: /entity_autodedup yes")
    return "\n".join(lines)[:4000]


async def entity_autodedup_handler(update, context):
    """/entity_autodedup [yes] — прохід по всій базі органів.

    Без `yes` нічого не змінює: показує числа й віддає повний список файлом."""
    if not _allowed(update):
        return
    if not bot_db.is_configured():
        await update.message.reply_text("🦊 Нора недоступна (BOT_DATABASE_URL).")
        return
    go = bool(context.args) and context.args[0].lower() in ("yes", "так")
    msg = await update.message.reply_text("🦊 Рахую групи…")
    try:
        groups, held = await asyncio.to_thread(autodedup_groups)
    except Exception as e:
        await msg.edit_text(f"❌ Не порахував: {type(e).__name__}: {e}")
        return
    if not groups:
        await msg.edit_text("🦊 Груп немає — ключ не зводить жодної пари.")
        return
    if not go:
        await msg.edit_text(_autodedup_text(groups, held))
        import io
        try:
            data = await asyncio.to_thread(autodedup_csv, groups)
            await update.message.reply_document(
                document=io.BytesIO(data), filename="entity_autodedup.csv",
                caption="Повний список: що з чим зіллється.")
        except Exception as e:
            await update.message.reply_text(f"(списком не віддалось: {e})")
        return
    await msg.edit_text(f"🦊 Зливаю {sum(len(g['losers']) for g in groups)} карток…")
    who = update.effective_user.full_name if update.effective_user else None
    try:
        res = await asyncio.to_thread(autodedup_apply, groups, who)
    except Exception as e:
        await msg.edit_text(f"❌ Прохід упав: {type(e).__name__}: {e}")
        return
    fail = ("\n\n❌ не вийшло:\n" + "\n".join(res["failed"][:5])) if res["failed"] else ""
    await msg.edit_text(
        f"🦊 Злито {res['done']} карток.\nПрогін: `{res['run']}`\n"
