...
# JUDGE_MIN свідомо низький (0.18): реальний кейс «шлам МГЗ» — підпис не
# переиспользував лід (свій заголовок + цитата з ТІЛА статті) і мав 0.276 при
# старому порозі 0.28 — рілз не дійшов навіть до судді. Обманки після стемінгу
# ≤0.17, а хибне потрапляння в сіру зону коштує лише один дешевий виклик
# Haiku, який скаже «0».
ACCEPT = 0.50      # ≥ — впевнений збіг без AI
NEAR = 0.10        # інші кандидати в межах ACCEPT..(best−NEAR) теж рахуємо збігом (пост+рілз)
# Нижній поріг судді — лише СИГНАЛЬНИЙ (хоч якийсь лексичний перетин, щоб не
# тягати зовсім випадкові пости): реальний кейс TikTok «шлам МГЗ» — SMM дав
# той самий відеоряд із принципово ІНШИМ підписом (про шторм, без слова про
# шлам), score 0.107. Такі вирішує тільки смисловий суддя, тому все, що не
# дотяглось до ACCEPT, іде до нього — зайвий виклик Haiku коштує долі цента.
JUDGE_MIN = 0.05
...
# Легкий стемінг: жадібна зрізка типових падежних закінчень (укр/рос) до
# основи ≥3 símв. Повного стемера для української немає, а точні токени
# ріжуть перетин («пилу/пилом» ≠ «пил», «екологи» ≠ «еколог», «негоди» ≠
# «негода») — реальний кейс «шлам МГЗ». Груба зрізка тут безпечна: скоринг
# працює на множинах слів, а не на морфології.
_SUFFIXES = sorted(
    ("іями", "ями", "ами", "ові", "еві", "ому", "ими", "іми", "ої", "ій",
     "ий", "ою", "ею", "єю", "ям", "ам", "ах", "ях", "ів", "їв", "ей",
...
def _norm_tokens(text):
    ...
    return [_stem(t) for t in _WORD_RE.findall(text) if len(t) >= 3 and t not in _STOP]
...
async def _judge(sig, candidates, platform="Instagram"):
    ...
    prompt = (
        ...
        "Який допис розповідає про ЦЮ САМУ новину чи подію? Підпис може "
        "переказувати сюжет іншими словами або описувати відео без деталей "
        "статті — важливо, чи це та сама подія (той самий випадок, місце, "
        "час), а не просто схожа тема. Відповідай ЛИШЕ числом — номером "
        "допису зі списку. Якщо жоден не про цю подію — відповідай 0. "
