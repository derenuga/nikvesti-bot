...
def _due(cur, weekly, now, ignore_sent):
    ...
    Джерело — РОБОЧИЙ ШАР (`pp.list_queue` за замовчуванням): у канал, який
    редакція читає поруч із тендерами, процедурні кроки й рутина не мають
    права піти взагалі. Стеля DAILY_MAX від цього не рятувала б — вона лише
    розтягнула б шум на місяці.
    """
