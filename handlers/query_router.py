...
from handlers import (analytics_store, archive_search, back_export, budget_nlq, card_maker,
                      news_archive, news_stats, social_store, storage, tg_html)
...
def _nlq_facebook_stats(period_days=7, start_date=None, end_date=None, _progress=None):
    ...
    if not stats:
        ...
        metrics_window = ("фіксований останній тиждень Meta — діапазон по днях "
                          "не віддався (фолбек)")
    # Сторінок листингу — за довжиною вікна: ~15-20 постів/день, 100 на
    ...
    def fmt_post(p):
        ...
        # ПЛОСКИМИ числами з insights (reactions/comments_count/shares_count),
        # а не вкладеними summary-об'єктами листингу — стара розкладка падала
        # тут з "'int' object has no attribute 'get'", і весь FB-tool виглядав
        # для моделі як «технічна проблема на боці Facebook».
        return {
            ...
            "shares": p.get("shares_count") or 0,
...
def _nlq_instagram_stats(period_days=7, start_date=None, end_date=None):
    ...
    return {
        ...
        "reach": window_stats.get("reach"),
        "views": window_stats.get("views"),
...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "УВАГА: це топ за ПЕРЕГЛЯДАМИ, а не перелік опублікованого за період — для 'що вийшло "
            "сьогодні/вчора і хто автор' бери list_news (БД сайту, дата публікації)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Статистика Facebook-сторінки МикВісті: підписники, перегляди контенту і "
            "взаємодії за вказаний період (сума по днях, поле metrics_window каже вікно), "
            "топ-5 публікацій і топ-5 рілзів за той самий період "
    ...
    },
    {
        ...
        "description": (
            ...
            "опубліковано (пости/рілзи/каруселі), перегляди/охоплення/взаємодії за той самий "
            "період (поле metrics_window каже вікно), топ-5 публікацій за лайками+коментарями. "
            "Використовуй для питань 'як інста?', 'скільки підписників прийшло?', "
            "'який рілз залетів?'."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "У кожного результату є excerpt — речення З ТЕКСТУ матеріалу навколо збігу: по ньому видно, "
            "ЧОМУ стаття знайшлась і що там насправді сказано. Саме так перевіряються питання на перетин "
            "('чи критикував X роботу Y', 'що Z заявив про W'): прізвище рідко буває в заголовку, а заява "
            "лежить у тілі тексту, тому шукай ПАРУ одним запитом ('Рябченко Миколаївські парки') і читай excerpt. "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "date_from": {"type": "string", "description": "Опційно: шукати з цієї дати, 'YYYY-MM-DD'. Роками свіжість не висловити — для «свіжих/останніх» новин і беку до сьогоднішнього матеріалу став вікно саме тут (напр. останні 3 місяці)"},
                "date_to": {"type": "string", "description": "Опційно: по цю дату включно, 'YYYY-MM-DD'"},
                "spread_years": {"type": "boolean", "description": "true — розкидати результати по роках (історія питання), false (дефолт) — найрелевантніші, при цьому найсвіжіші за датою збіги гарантовано в списку (для 'що останнє про X' дефолт підходить)"},
                ...
                "region": {"type": "integer", "description": "Опційно: код регіону (1 Миколаїв, 2 Україна, 3 Світ, 4 Херсон, 5 Одеса)"},
            },
            "required": ["query"],
        },
    },
    ...
    {
        "name": "get_leads_from_urls",
        "description": (
            "Ліди новин за їхніми URL nikvesti.com — щоб скласти бек із конкретних "
            "посилань, які надіслав користувач (а не з пошуку). Викликай, коли у "
            "повідомленні є посилання на nikvesti.com і просять зробити з них бек. "
            "Передай усі посилання масивом urls як є. Повертає items з лідами, вже "
            "відсортовані свіжіше→давніше (порядок за датами новин, НЕ за порядком "
            "посилань) + missing (посилання, які не знайшлись у базі)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "urls": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Повні URL новин nikvesti.com з повідомлення користувача",
                },
            },
            "required": ["urls"],
        },
    ...
    {
        "name": "get_article_for_post",
        "description": (
            "Дані нашої новини (заголовок, лід, лінк) для написання ПОСТА в соцмережі. "
            "Викликай, коли просять зробити/написати пост, анонс чи підводку для "
            "Facebook, Telegram чи Instagram із посилання на nikvesti.com "
            "(«зроби пост в фейсбук», «напиши анонс для телеграма»). Це НЕ бек: "
            "бек нагадує історію теми з кількох новин, а пост — коротка підводка до "
            "ОДНІЄЇ новини. Під відповіддю з'явиться кнопка в генератор карток."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "url": {
                    "type": "string",
                    "description": "Повний URL новини nikvesti.com із повідомлення користувача",
                },
            },
            "required": ["url"],
        },
    ...
    },
    {
        ...
        "description": (
            ...
            "count_archive_by_month (там пошук і по тексту). Якщо треба не число, "
            "а ПЕРЕЛІК матеріалів (які саме, із заголовками/авторами/лінками) — "
            "бери list_news."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        "name": "list_news",
        "description": (
            "ПЕРЕЛІК опублікованих матеріалів nikvesti.com за період — прямо з БД сайту, "
            "за ДАТОЮ ПУБЛІКАЦІЇ (свіже, джерело істини): заголовок, час виходу, автор, "
            "лінк і ознака own (власний/рерайт) для кожного. Використовуй для питань "
            "'що ми сьогодні опублікували', 'скільки у нас сьогодні своїх матеріалів і "
            "які, хто автор', 'що вийшло вчора', 'покажи матеріали Юлі за тиждень'. "
            "own_material=true — лише власні матеріали редакції. total у відповіді — "
            "скільки всього матчить фільтрам (навіть якщо в items показано менше): "
            "на 'скільки і які' це відповідь ОДНИМ викликом, count_news не потрібен. "
            "НЕ плутати з get_ga4_top_articles — той дає топ за переглядами GA4, куди "
            "потрапляють і старіші матеріали, які досі читають."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "period": {
                    ...
                    "description": "За замовчуванням today. 'custom' — разом зі start_date/end_date",
                },
                "own_material": {"type": "boolean", "description": "true — лише власні матеріали редакції (own_material=1). Ставити, коли питають саме про власні/свої/оригінальні"},
                "author": {"type": "string", "description": "Опційно: ім'я або прізвище автора (напр. 'Середа', 'Юлія Бойченко')"},
                "category": {"type": "string", "description": "Опційно: слаг рубрики (напр. 'politics')"},
                "limit": {"type": "integer", "description": "Скільки матеріалів повернути, за замовчуванням 30, максимум 60"},
                "start_date": {"type": "string", "description": "YYYY-MM-DD, тільки якщо period='custom'"},
                "end_date": {"type": "string", "description": "YYYY-MM-DD, тільки якщо period='custom'"},
            },
            "required": [],
...
TOOL_FUNCTIONS = {
    ...
    "list_news": news_stats.list_news,
    "get_news_leads": news_archive.get_news_leads,
    "get_leads_from_urls": news_archive.fetch_items_by_urls,
    "get_article_for_post": news_archive.get_article_for_post,
...
def _meta_error_details(msg):
    """Класифікація збою Graph API для чесної відповіді користувачу: протухлий
    токен — це не «тимчасова проблема на боці Meta», а конкретна дія для Олега,
    і модель має переказати її, а не вгадувати причину. Патерни — з реальних
    повідомлень OAuthException (code 190: Error validating access token /
    Session has expired / The access token could not be decrypted)."""
    m = (msg or "").lower()
    token_signs = (
        "access token", "session has expired", "oauth",
        "code 190", "(#190)", "token could not be decrypted",
    )
    if any(s in m for s in token_signs):
        return {
            "cause": "token_expired",
            "hint": (
                "Схоже, протух токен Meta (FACEBOOK_PAGE_TOKEN для Facebook, "
                "INSTAGRAM_TOKEN для Instagram). Полагодити може лише Олег "
                "(@derenuga): https://developers.facebook.com/tools/explorer/ → "
                "обрати застосунок → Get Page Access Token (для сторінки МикВісті) → "
                "подовжити до довгоживучого через Access Token Debugger "
                "(https://developers.facebook.com/tools/debug/accesstoken/, кнопка "
                "Extend Access Token) → оновити відповідну змінну на Railway."
            ),
        }
    return {
        "cause": "meta_side",
        "hint": (
            "Схоже на тимчасовий збій Meta або мережі — можна спробувати ще раз "
            "за кілька хвилин. Якщо повторюється довше години — сказати Олегу."
        ),
    }
...
TOOL_PROGRESS = {
    ...
    "list_news": "🦊 Гортаю стрічку публікацій...",
    "get_news_leads": "🦊 Перечитую ліди цих новин...",
    "get_article_for_post": "🦊 Читаю новину і складаю пост...",
...
Для ГРАФІКА по бюджету: query_type='execution_trend' — динаміка % виконання видатків по місяцях («як місто освоює бюджет протягом року», «графік виконання») → одразу render_chart chart_type='line' з labels і values. query_type='budget_growth' — як рішення сесії нарощували бюджет (млрд грн по ревізіях) → render_chart line, дві серії expenditure_bln/revenue_bln. Числа з values/labels НЕ переказуй довгим списком — коротка підводка + графік.
...
КОЛИ ПОКАЗУЄШ ТОП СТАТЕЙ ЗА ПЕРЕГЛЯДАМИ (get_ga4_top_articles) — нумерований список, кожна стаття окремим пунктом із двох рядків у форматі (HTML):
1. <a href="URL">Заголовок</a> — 35 436
👤 Світлана Іванченко · ✍️ власна
Заголовок стає гіперлінком: URL бери РІВНО з поля url результату, символ-у-символ, слаг не вигадуй і не змінюй. Число переглядів — з пробілом між розрядами тисяч, слово «переглядів» у кожному рядку не повторюй (скажи один раз у підводці). Другий рядок: 👤 ім'я автора (поле author), далі « · » і позначка власності з поля own: ✍️ власна (true) або 🔄 рерайт (false); own=null — позначку не став, author=null — пропусти ім'я; якщо немає ні автора, ні позначки — другого рядка не пиши. Якщо викликав з own_only=true — усі статті власні, ✍️ в кожному рядку не повторюй, натомість зазнач у підводці, що це топ саме власних матеріалів. Символи & < > у заголовках заміни на &amp; &lt; &gt;. І пам'ятай: власність визначає ЛИШЕ поле own — не суди з підпису автора.
...
Якщо питають, що ми писали про когось/щось ("що ми останнє писали про Сєнкевича?", "що було про завод Океан?", "що по мерії?") — використай search_archive_fulltext (шукає в заголовках І повному тексті всієї 17-річної історії). query — базові форми слів ("Сєнкевич", "завод Океан"). Якщо питають про ДВІ (чи більше) теми/персони одразу ("що писали про A та B") — СПОЧАТКУ зроби один пошук по обох разом (query з обома словами: "Панченко Кантор" → матеріали, де є і те, і те — їхня взаємодія). Якщо спільних матеріалів замало — доший окремо по A і окремо по B; ці пошуки автоматично зливаються в ОДИН список з наскрізною нумерацією (items останнього виклику = повний список). У будь-якому разі показуй результат ОДНИМ суцільним нумерованим списком з наскрізними номерами (1,2,3…), а НЕ двома блоками, де нумерація починається з 1 двічі — кнопки відбору мають єдину нумерацію, тож два списки по «1-10» роблять кнопки неоднозначними. ЗА ЗАМОВЧУВАННЯМ spread_years НЕ став — тоді список іде найсвіжіше зверху (звичайний "що ми писали про X" / "що останнє писали" саме такий). spread_years=true став ТІЛЬКИ коли явно просять таймлайн через усі роки ("вся історія питання", "що взагалі було з X за всі ці роки", "від початку і донині") — цей режим сортує від давніх до свіжих і бере лише кілька на рік, тому для звичайного запиту він і перевертає хронологію, і витісняє свіжі матеріали старими (та ще й старіші роки часто лише російською). Якщо результатів мало — повтори з російським написанням (старі матеріали російською: "Сенкевич") або синонімом. Якщо search_archive_fulltext повертає помилку про неналаштоване дзеркало — використай search_news_archive (пошук по заголовках; у keywords — основи слів без закінчень). Відповідь — нумерований список, кожна новина ОДНИМ рядком у форматі:
...
БУДЬ-ЯКИЙ перелік новин у твоїй відповіді — і той, що ти показуєш замість беку («прямого підтвердження не знайшов, але є ось що»), і будь-який інший — іде ТИМ САМИМ форматом, із гіперлінком на заголовку і номерами n з items. Перелік голим текстом марний: журналістка не може ні відкрити новину, ні скопіювати її в матеріал, і кнопки відбору під таким списком показують не ті номери.
...
Якщо питають ЩО САМЕ вийшло за період — "що ми сьогодні опублікували?", "скільки у нас сьогодні своїх матеріалів? які і хто автор", "що вийшло вчора?", "покажи матеріали Каті за тиждень" — бери list_news (перелік прямо з БД сайту за датою публікації: заголовок, час виходу, автор, лінк, ознака own). own_material=true — коли питають саме про власні/свої. НЕ бери для цього get_ga4_top_articles: то топ за переглядами GA4, а не опубліковане за період (туди потрапляють і старіші матеріали, які досі читають). На "скільки і які" відповідай одним викликом list_news — поле total і є кількість, окремий count_news не потрібен. Відповідь — коротка підводка з числом (total) і нумерований список, кожен матеріал двома рядками у форматі (HTML):
1. 🕐 14:32 — <a href="URL">Заголовок</a>
👤 Автор
...
Якщо просять бек/нагадування, ПРИВʼЯЗАНЕ ДО ПЕРСОНИ («збери нагадування про критику від Рябченка на роботу КП „Миколаївські парки"», «що Панченко казав про хлібозавод», «як мер коментував бювети») — це запит на ПЕРЕТИН двох речей, і шукати треба перетин, а не одну з них:
1) ПЕРШИЙ пошук — по ПАРІ разом (query "Рябченко Миколаївські парки"). Прізвище в заголовку буває рідко, а слова людини лежать у ТІЛІ тексту — саме там їх бачить search_archive_fulltext. Пошук лише по об'єкту («Миколаївські парки») на це питання не відповідає взагалі: він знайде тему, але не персону.
2) Читай excerpt кожного збігу — це речення з тексту навколо збігу, і саме по ньому видно, чи справді ця людина це казала. НЕ роби висновку «не згадується» з того, що імені немає в заголовках або лідах: лід — лише перший абзац, а заява майже завжди в середині тексту.
3) Пара нічого не дала — спробуй інші написання (саме прізвище без імені; російське написання; «депутат Рябченко») і лише після цього кажи прямо: у наших матеріалах такого не знайшов.
4) І тоді ж покажи, що Є по об'єкту — звичайним нумерованим списком з лінками — і одним рядком спитай, чи скласти бек по ньому. Не приписуй людині слів, яких не бачив у excerpt чи ліді (це правильно), але й не відмовляйся, не зробивши крок 1.
Той самий шлях — для будь-якого «А про Б»: хто кого критикував, хто що обіцяв, хто на кого скаржився, як хтось голосував.
...
СВІЖІСТЬ У БЕКУ — вирішуй сам, не перепитуй. На «вічних» темах (обстріли, евакуація, ремонт доріг, комунальні аварії, ДТП) релевантний збіг ≠ свіжий: пошук ранжує за релевантністю, тому в списку легко опиняється торішній матеріал, а журналістці потрібні останні тижні. Тому:
1) Бек складається ДО СВІЖОЇ новини (дали заголовок сьогоднішнього матеріалу чи лінк на нього) АБО просять «свіжі», «останні», «найновіші» без явного періоду → одразу став вікно date_from ≈ 3 місяці тому (порахуй від сьогоднішньої дати вище), date_to не потрібен. У search_news_archive той самий важіль зветься period_days (≈90).
2) У вікні знайшлось замало (менше 3 новин) — розшир вікно (пів року, рік) і ОДНИМ рядком скажи це людині: «свіжіших за червень у нас немає, тому взяв ще травневі». Мовчки підмішувати старе до «свіжого» не можна — саме на цьому доводиться перепитувати «який травень?».
3) Явний період у питанні («за липень-серпень», «торік», «з початку року») завжди сильніший за це правило: рахуй дати і став date_from/date_to рівно по ньому.
4) Уточнили вже ПІСЛЯ беку («тільки свіжі», «новини за серпень липень») — не шукай наново по всьому архіву: повтори пошук із вікном і перескладай бек лише з того, що в нього влізло.
ЯКЩО ПРОСЯТЬ ПОСТ У СОЦМЕРЕЖІ («зроби пост в фейсбук», «напиши анонс для телеграма», «підводка для інсти») і дали посилання на нашу новину — виклич get_article_for_post з цим URL і напиши пост. Це НЕ бек: бек нагадує історію теми, пост — коротка підводка до ОДНІЄЇ новини, щоб людина захотіла її відкрити. Формат відповіді:
Готовий пост для Facebook:
<pre>Перше речення — суть новини або гачок.
Друге-третє — конкретика з ліду: число, місце, хто вирішив.

🔗 URL</pre>
Правила поста: 2-4 короткі речення живою мовою, тільки факти з title і lead — нічого не додумуй; заголовок дослівно не копіюй, а перефразуй (у стрічці ФБ дубль заголовка й лінка виглядає порожньо); лінк — ГОЛИЙ URL з поля url окремим рядком, НЕ гіперлінк (у Facebook розмітки немає, пост копіюють як звичайний текст, тому весь пост іде всередині <pre>, а всередині нього жодних тегів); емодзі 0-2, клікбейту й капслоку немає; хештеги лише якщо просять. Після блоку — не більше одного рядка від себе. Кнопка «Зробити картку» під відповіддю з'явиться сама — про неї не пиши.
...
URL бери РІВНО з поля url результату, символ-у-символ, нічого в ньому не міняй. Опис у лінку — коротка суть з text/caption (5-8 слів). Емодзі за типом: 🎬 рілз/відео (VIDEO), 🖼 карусель (CAROUSEL_ALBUM), 📷 фото (IMAGE), 📝 пост ФБ. Якщо url у публікації відсутній (null) — рядок без лінка, простим текстом. Символи & < > в описах заміни на &amp; &lt; &gt;.
Якщо просять ПОЯСНИТИ СТРИБОК чи провал соцмереж («чому в червні взаємодії ФБ виросли на 445%?», «що сталось з охопленням у липні?») — зроби ДВА виклики того самого tool з start_date/end_date обох періодів (місяць-пік і місяць-база), порівняй зведені числа і топи: зазвичай відповідь — конкретні вірусні публікації, назви їх лінками з числами взаємодій. Будь чесним щодо межі даних: топ постів ФБ рахується лише по постах із посиланням на nikvesti.com (рілзи — окремим топом), а page_post_engagements містить УСЮ активність сторінки — тож якщо топи стрибка не пояснюють, так і скажи, що джерело зростання поза постами з лінками (нативні пости, фото, шери). Це аналіз «що вистрілило», а не повна атрибуція.
Якщо get_facebook_stats або get_instagram_stats повернув error — НЕ вигадуй причину сам, дивись поля cause/hint: cause="token_expired" означає, що протух токен Meta і його має оновити Олег — перекажи hint дослівно, разом із лінками; cause="meta_side" — тимчасовий збій, чесно скажи спробувати пізніше. Якщо друга мережа при цьому відповіла — її дані все одно покажи повністю.
...
async def handle_natural_language_query(update, context):
    ...
    lead_items_seen = []
    # Новина, для якої Лис написав ПОСТ у соцмережі (get_article_for_post) —
    # під такою відповіддю кнопка в генератор карток, а не копіювання в Docs.
    post_article_id = None
    ...
    # Стеля токенів на відповідь. За замовчуванням дешева (4096); якщо перша
    # спроба обірвалась на max_tokens (важка арифметика з'їла бюджет на
    # роздуми+текст), один раз піднімаємо стелю і просимо стиснути — щоб не
    # спихати на користувача "звузь період", а самому дати короткий підсумок.
    current_max_tokens = ROUTER_MAX_TOKENS
    concise_retry_done = False
    # Додатковий system-блок для стислої переспроби (порожній у звичайному режимі)
    concise_directive = None
    ...
    try:
        for _ in range(MAX_TOOL_ITERATIONS):
            ...
            if concise_directive:
                system_blocks.append({"type": "text", "text": concise_directive})
            ...
            if response.stop_reason != "tool_use":
                ...
                if not final_text:
                    ...
                    # max_tokens = роздуми+викладки з'їли весь бюджет, на текст не
                    # лишилось. Замість заглушки "звузь період" даємо моделі ще
                    # один прохід із вищою стелею і жорсткою вимогою стислості —
                    # хай сама стисне (усі tool-дані вже зібрані в messages,
                    # переспроба лише формулює короткий підсумок).
                    if response.stop_reason == "max_tokens" and not concise_retry_done:
                        concise_retry_done = True
                        current_max_tokens = 8192
                        concise_directive = (
                            "ВАЖЛИВО: попередня спроба відповіді обірвалась, бо не "
                            "вмістилась у ліміт токенів. Усі потрібні дані вже зібрані — "
                            "нових запитів до tools НЕ роби. Відповідай МАКСИМАЛЬНО стисло: "
                            "6-8 коротких рядків, лише ключові числа й один висновок. "
                            "Не переказуй методику, не показуй проміжні обчислення по днях, "
                            "не порівнюй сам процес — тільки готовий результат."
                        )
                        await _update_placeholder(
                            placeholder, "🦊 Стискаю відповідь...", last_placeholder_text)
                        continue
                ...
                if used_tools:
                    ...
                    if (used_tools & NEWS_TOOL_NAMES or "count_archive_by_month" in used_tools
                            or "count_news" in used_tools or "list_news" in used_tools
                            or "get_leads_from_urls" in used_tools):
                ...
                reply_markup = None
                if post_article_id:
                    # Відповідь — ПОСТ у соцмережі. Наступний крок після тексту
                    # завжди той самий: картинка. Та сама кнопка, що в монітора
                    # новин без ФБ, і той самий генератор карток.
                    reply_markup = card_maker.card_button(post_article_id)
                elif used_tools & SEARCH_TOOL_NAMES and "get_news_leads" not in used_tools:
                ...
                # <b> (блоки змін бюджету) або <pre> (готовий пост для соцмереж —
                # блок «тапнув і скопіював»). Інакше теги показуються голим
                # текстом. Без розмітки — простий edit_text (менше шансів на 400).
                has_html = ("<a href=" in final_text or "<b>" in final_text
                            or "</b>" in final_text or "<pre>" in final_text)
            ...
            for block in response.content:
                ...
                try:
                    ...
                    result = {"error": str(e)}
                    # Для Meta-tools додаємо причину людською мовою: без неї
                    # модель бачить голий текст винятку і списує все на
                    # «проблеми на боці Facebook», навіть коли справа в
                    # протухлому токені (який лагодиться конкретною дією).
                    if block.name in META_TOOL_NAMES:
                        result.update(_meta_error_details(str(e)))
                if block.name == "render_chart" and isinstance(result, dict) and result.get("path"):
                    chart_path = result["path"]
                # Копимо реальні новини (id+url) з news-tools — для санітизації
                # лінків фінальної відповіді (модель НЕ сміє вигадувати слаги).
                # Топ GA4 теж лінкується (заголовки-гіперлінки) — його url у ту
                # саму скарбничку, інакше санітизатор розлінкує їх як невідомі.
                if (block.name == "list_news" and isinstance(result, dict)
                ...
                if (block.name == "get_ga4_top_articles" and isinstance(result, dict)
                        and isinstance(result.get("articles"), list)):
                    news_items_seen.extend([
                        a for a in result["articles"]
                        if isinstance(a, dict) and a.get("id") and a.get("url")
                    ])
                # Новина для поста: id — на кнопку картки, url — у скарбничку
                # санітизації (якщо модель усе ж зробить із лінка анкор).
                if (block.name == "get_article_for_post" and isinstance(result, dict)
                        and isinstance(result.get("article"), dict)):
                    article = result["article"]
                    if article.get("id"):
                        post_article_id = article["id"]
                    if article.get("id") and article.get("url"):
                        news_items_seen.append(article)
