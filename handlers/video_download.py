...
Про YouTube потрібне уточнення, бо саме на ньому легше за все зробити хибний
висновок в ОБИДВА боки. Заміряно двічі:
  • одне відео («Me at the zoo») дало «Sign in to confirm you're not a bot» на
    ВСІХ дев'яти player-клієнтах — тобто один ролик здатен збрехати, що весь
    сервіс закритий;
  • і навпаки: у ту саму секунду з тієї самої адреси Rick Astley качався, а
    ролик, який приніс Олег, вимагав підтвердження (замір 14.08).
Отже перевірка на робота — ПОРОЛИКОВА, а не «YouTube не качає без логіна».
Більшість відео качається одразу. Через це сторінка мусить розрізняти дві
причини відмови (refusal_kind): «перевірка на робота» і «треба бути
залогіненим» — сказати перше словами другого означає збрехати людині, яка
щодня качає ті самі відео через savefrom (реальна претензія Олега 14.08).

Чому такі сервіси проходять там, де ми ні: у них пул проксі й підмінних
адрес, це буквально їхній продукт. У нас один сервер з однією адресою.
Наші виходи, у порядку дешевизни: куки (робиться раз), YTDLP_PROXY
(резидентний проксі — найближче до того, як працює savefrom), або окремий
сервіс-постачальник PO-токенів (найважче, зате без чужих кук).
...
Звідси конструкція (питання Олега «а що не можна просто звичайну сторінку без
виклику через чат?» — можна, і ось як саме):
  • СТОРІНКА ВІДКРИТА, як /card і /back: качати публічне може будь-хто, хто
    має адресу, і ключ для цього не потрібен ніколи. YouTube і відео сторінок
    Facebook працюють без жодних кук, і саме вони — головне навантаження;
  • ключ (`?k=…`) — це НЕ пропуск, а памʼять про те, чий доступ брати. Він
    народжується мовчки перед першим збереженням кук і живе в localStorage
    браузера: ніде не треба ні входити, ні пересилати лінк. Через це
    випадковий відвідувач не заводить у стані навіть запису;
  • хто дав СВІЙ доступ — качає під собою; хто ні — під редакційними (якщо
    їх узагалі клали) або зовсім без кук;
  • на інший пристрій ключ переїжджає кнопкою «Відкрити на телефоні» — саме
    тому дозвіл, даний раз із компʼютера, працює далі й з телефона;
  • команда /video лишається зручним входом із чату (де лінк зазвичай і
    лежить) і видає ІМЕННИЙ ключ — тоді в журналі видно, хто качає.

Окремо про Safari: розширення для експорту кук під нього НЕ ІСНУЄ — ні на
Mac, ні на iPhone. Це сказано прямо в майстрі, з виходом: поставити Chrome
або Firefox рівно на один раз, бо доступ далі живе на сервері.
...
def refusal_kind(text):
    """ЧОМУ відмовили — бо перевірка на робота чи бо треба бути залогіненим.

    Різниця не косметична, і сплатив за неї Олег (14.08): сторінка сказала
    «це відео віддається лише тому, хто залогінений» на звичайне відео
    YouTube — і це читається як «YouTube взагалі не качається без логіна»,
    що неправда. Заміряно тієї ж секунди з тієї ж адреси: одне відео
    качається, друге вимагає підтвердження. Тобто:
      botcheck — YouTube увімкнув суворішу перевірку САМЕ на цьому ролику
                 (більшість качається одразу), і сервер має довести, що він
                 не робот. Сайти-качалки проходять це десятками проксі —
                 у нас одна адреса;
      login    — відео справді віддається лише залогіненому (рілзи Facebook,
                 Instagram, приватні відео).
    """
    low = (text or "").lower()
...
def humanize_error(text, url=""):
    ...
    if "not a bot" in low or "sign in to confirm" in low:
        return (f"{who} увімкнув суворішу перевірку саме на цьому відео й "
                f"просить сервер підтвердити, що він не робот. Більшість "
                f"відео качається одразу; це лікується дозволом качати від "
                f"твого імені — робиться один раз.")
