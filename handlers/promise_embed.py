...
APART = [
    ...
    # Пара переїхала СЮДИ з «одна справа» 22.08, і це виправлення мого ж
    # набору, а не поступка провайдерам. Обидва замiри вперлись саме в неї
    # («найважчий справжній збіг»), тож я подивився в записи: 1961 —
    # «Підписати грантову угоду з НЕФКО…», обіцяльник НЕФКО, kind=process,
    # тобто ЖИВЕ В ТІНІ й у черзі не буває взагалі; 1999 — «Реконструювати
    # систему водовідведення у Південноукраїнську», обіцяльник міська рада,
    # kind=commitment. Різний обіцяльник і різний тип акту — за НАШИМИ
    # правилами це не дубль, і звести їх означало б зіпсувати банк, а не
    # полагодити. Отже еталон був хибний, а не інструмент сліпий.
    ((1961, 1999), "НЕФКО: підписати угоду (тінь) / реконструювати (черга)"),
]
...
PROVIDERS = {
    ...
    "Google · gemini-embedding-001 (similarity)": {
        ...
        # Порція вдвічі більша, ніж була: прогін 22.08 упав на 429 навіть із
        # витримкою, тобто впирається не лише в токени за хвилину, а й у
        # к-сть ЗАПИТІВ. 858 записів це 9 запитів замість 18.
        "chunk": 100,
        "pace": 12.0,
    },
