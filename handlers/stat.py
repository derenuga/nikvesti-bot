...
from handlers.facebook import (
    get_reel_insights, fix_permalink,
    POST_LIST_FIELDS, graph_get as _graph, parse_post_insights as _parse_post_insights,
    get_post_metrics as _get_post_metrics,
)
...
async def stat_handler(update, context):
    ...
    if not (fb_idx and ig_idx and tt_idx and yt_idx):
        ...
        # Друга сигнатура: підпис, який СММ забрала з генератора каруселей.
        # Заголовок і лід — це те, що ми ПРИПУСКАЄМО в дописі; цей текст у
        # дописі справді лежить, тож у сірій зоні він вирішує без судді.
        if sig:
            from handlers import storage
            sig["caption"] = storage.get_carousel_caption(article_id)
