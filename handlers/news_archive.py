...
# Підпис блоку — РЕЧЕННЯ, а не ярлик. Попередній звучав «📎 Згадані матеріали
# без лінка в тексті», і Олег справедливо спитав (12.08), що це взагалі таке:
# з нього не видно ні звідки взялись ці новини, ні чому їх більше, ніж у
# списку вище. Тепер підпис сам себе пояснює: це новини, які Лис ЧИТАВ, щоб
# відповісти, і на які він не поставив посилання в тексті.
MISSING_LINKS_HEADER = (
    "🔗 Новини, які я переглянув для цієї відповіді, "
    "але посилання в текст не поставив:")


def append_missing_links(text, items):
    """Детермінований дожим лінків: новини з items (ті, чиї ЛІДИ читались —
    get_news_leads / get_leads_from_urls), чий url НЕ зʼявився в тексті
    відповіді, дописуються окремим блоком з канонічними лінками.
    ...
    Блок буває більшим за список у відповіді — і це не збій: сюди йдуть УСІ
    прочитані новини без лінка, а Лис у тексті міг згадати не всі (кейс 12.08:
    у відповіді 7 новин голим текстом, у блоці 10 — бо ліди читались по
    десятьох). Правильне лікування того кейсу — не тут, а в промпті: будь-який
    перелік новин має йти з гіперлінками, і тоді блок просто не спрацьовує."""
    if not text or not items:
        return text
...
def get_article_for_post(url):
    """Дані ОДНІЄЇ новини для написання поста в соцмережі (NLQ-tool).

    Той самий резолв URL→новина, що й для беку (fetch_items_by_urls), але
    окремим tool'ом свідомо: за назвою tool'а роутер розуміє, що людина
    просила ПОСТ, а не бек, — і чіпляє під відповідь кнопку в генератор
    карток замість кнопки копіювання в Docs. Дві різні задачі — дві різні
    наступні дії, і плутати їх по одному й тому самому виклику не можна."""
    result = fetch_items_by_urls([url])
    items = result.get("items") or []
    if not items:
        return {"error": result.get("error") or f"Новина за посиланням {url} не знайшлась у базі."}
    return {
        "article": items[0],
        "note": (
            "Це наша опублікована новина: title — заголовок, lead — перший "
            "змістовний абзац, url — канонічне посилання. Пиши пост ТІЛЬКИ з "
            "цих фактів, нічого не додумуй. Кнопка «Зробити картку» під "
            "відповіддю з'явиться сама — не згадуй її в тексті."
        ),
    }
