...
- тиждень, повністю накритий відпусткою → ціль 0;
...
def main():
    ...
    # ---------- 5а. Написане у відпустці зараховується ----------
    # Питання Олега: «а якщо людина щось напише, будучи у відпустці?».
    # Факт береться з дати публікації, відпустка знижує лише ціль — тож
    # написане рахується. І навіть коли період накрито цілком (ціль 0), вона
    ...
    try:
        ...
        orig_bounds = team_kpi.period_bounds
        team_kpi.period_bounds = lambda p, off=0, t=None: orig_bounds(p, off, today)
        dash = team_kpi.kpi_dashboard("week", 0)
        team_kpi.period_bounds = orig_bounds
        row = next((p for p in dash["people"] if p["person"] == ALINA), None)
        check("людина не зникла зі «Звіту», хоч тиждень у відпустці", row is not None)
        n0 = row["norms"][0] if row else {}
        check("написане у відпустці зараховано", n0.get("fact") == 2)
        check("ціль лишилась 0, а виконання — 100%",
              n0.get("target") == 0 and n0.get("pct") == 100 and n0.get("done"))
        check("позначено як «понад норму»", n0.get("beyond") is True)
    finally:
