...
# питання «чи це одна справа». Замір 17.08.2026 по серпню: 1420 пар детектора
# з 2287 народила саме ця картка, і вони дали ОДНЕ закриття (0.07%).
#
# Ловиться це ДВОМА ознаками разом, і кожна поодинці бреше:
#
# 1. **Підтип.** `street` і `facility` — завжди предмет, скільки б їх не
#    згадували: «проспект Центральний» має 408 згадок, але обіцянка «укласти
#    покриття на проспекті» — саме про нього. Тому вулиці й об'єкти не
#    відсікаються НІКОЛИ. Контейнером буває лише `settlement` (місто, село)
#    чи `district` (область, район).
# 2. **Частота.** Але й населений пункт сам по собі контейнером не є:
#    «Матвіївка» (103 згадки), «Варварівка» (126), «Намив» (128) — це саме
#    предмет обіцянки, і кейс, заради якого детектор писався 04.08, був якраз
#    про дорогу до Матвіївки. Контейнером населений пункт робить тільки те,
#    що він трапляється всюди.
#
# Разом ці дві ознаки сьогодні відсікають РІВНО ДВІ картки — «Миколаїв»
# (11 901) і «Миколаївська область» (10 536), — і жодної іншої. Правило при
# цьому самообслуговується: якщо колись до цієї межі доросте ще один
# топонім-фон, він відсіється сам, а нове написання вулиці не відсіється
# ніколи. Замір розкладу закриттів: із 35 живих переживає правило 34
# (єдина втрата — обіцянка про банери, що збіглася виключно по «Миколаєву»),
# а пар меншає з 1738 до 609.
#
...
def fulfil_candidates(cur, since, limit=200):
    ...
    **1. Спільна КАРТКА сутності.** Збіг рахується по картці, а не по тексту:
    ...
    справа». Такі картки-контейнери відсікають CONTAINER_PLACE_SUBTYPES +
    CONTAINER_PLACE_MENTIONS (там же заміряно, чому ознаки саме дві й чому
    вулиці не відсікаються ніколи). Обіцянка, у якої немає жодного об'єкта,
    крім самого міста, після цього пар по картках не отримує — і це
    правильно: якщо предмет не названо конкретніше за «Миколаїв», судити
    виконання нема по чому.
