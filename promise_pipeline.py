...
def merge_winner(a, b):
    ...
    Порядок: більше ревізій (довший ланцюг вести далі) → конкретніша назва
    (є число, власна назва або назва в лапках) → менший id, тобто той, з
    якого ланцюг почався.
    """
    def rank(r):
        return (int(r.get("revisions") or 0),
                1 if _title_is_specific(r.get("title") or "") else 0,
                -int(r["id"]))
