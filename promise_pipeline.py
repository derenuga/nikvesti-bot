...
# З якої частоти згадок картка МІСЦЯ перестає бути об'єктом обіцянки і стає
# просто тлом (див. fulfil_candidates). «Миколаїв» — не те, ЩО обіцяли, а те,
# ДЕ обіцяли: цю картку несе кожна миколаївська новина, тож збіг по ній нічого
# не означає. Замір 17.08.2026 по серпню (2287 пар детектора виконання):
#   місця <300 згадок — 201 пара, 10 закриттів (5%)
#   місця 300–1000    — 431 пара,  2 закриття (0.5%)
#   місця ≥5000       — 1420 пар,  1 закриття (0.07%)
# Тобто 1851 пара з 2287 (81% усієї ціни детектора) давала три закриття, з
# яких два суперечили власним правилам промпту («оголосили тендер» — це не
# виконання). Поріг стоїть НА МІСЦЯХ і тільки на них: організації тим самим
# заміром дали 29 пар на смузі 300–1000 («Миколаївводоканал»,
# «Миколаївоблтеплоенерго») — це справжні обіцяльники, і різати їх нема за що.
GENERIC_PLACE_MENTIONS = 300
...
def fulfil_candidates(cur, since, limit=200):
    ...
    **1. Спільна КАРТКА сутності.** Збіг рахується по картці, а не по тексту:
    ...
    різні рядки й одна картка. АЛЕ картка міста доказом не є: «Миколаїв» лежить
    в об'єктах сотень обіцянок і в тексті кожної миколаївської новини, тож
    збіг по ній означає лише «і те, і те про Миколаїв». Такі картки відсікає
    GENERIC_PLACE_MENTIONS — там же заміряно, чому поріг стоїть саме на
    місцях і саме на цьому числі. Обіцянка, у якої взагалі немає об'єкта,
    крім міста, після цього пар по картках не отримує — і це правильно: якщо
    предмет не названо конкретніше, судити виконання нема по чому.
    ...
    **2. Стаття, яку суддя ланцюга вже прив'язав до цієї обіцянки** пізнішою
    ...
    cur.execute(
        ...
        pairs AS (
            ...
            -- новина, і збіг по ній не каже нічого (див. докстрінг).
            JOIN entities e ON e.id = ae.entity_id
              AND NOT (e.kind = 'place' AND coalesce(e.mentions, 0) >= %s)
        ...
        LIMIT %s
        """,
        (GENERIC_PLACE_MENTIONS, int(since), int(since), int(since),
         int(limit)))
