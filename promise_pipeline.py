...
def populism_reason(row):
    """Підстава мітки «популізм» — рівно ті поля, які порожні (§2.1).

    Мітка НІКОЛИ не показується сама: це не вирок, а підсумок трьох порожніх
    полів, і людина має бачити, з чого він складений. Повертає None, коли
    мітки немає.
    """
    if row.get("verifiability") != "unfalsifiable":
        return None
    parts = []
    if not row.get("deadline"):
        parts.append("немає дати")
    if not (row.get("criterion") or "").strip():
        parts.append("немає критерію")
